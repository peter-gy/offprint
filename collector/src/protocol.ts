...
export function selectorInvalidError(
  captureId: string,
): CollectorProtocolError {
  return {
    type: "error",
...
export function selectorNotFoundError(
  captureId: string,
): CollectorProtocolError {
  return {
    type: "error",
  ...
  for (let block = 0; block < paddedBytes; block += 64) {
    ...
    for (let word = 16; word < 64; word += 1) {
      ...
      const before2 =
        ...
        (words[word - 2] >>> 10);
      words[word] =
        (words[word - 16] + before15 + words[word - 7] + before2) >>> 0;
    }
    ...
    for (let round = 0; round < 64; round += 1) {
      ...
      const first =
        (h + upper + choice + sha256RoundConstants[round] + words[round]) >>> 0;
