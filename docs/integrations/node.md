...
Use `captures.start` when the caller needs memory output, credentials, custom
network rules, the complete browser environment, diagnostics, or exact limits.
This complete request disables lazy-load scrolling to keep the first example
