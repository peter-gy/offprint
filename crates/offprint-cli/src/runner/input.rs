...
use offprint::{ArtifactSource, ErrorStage, OffprintError, Result};
use serde::de::DeserializeOwned;

pub(super) fn ensure_format_input_readable(path: &str) -> Result<()> {
...
pub(super) fn read_artifact(value: String, input: &mut dyn Read) -> Result<ArtifactSource> {
    if value == "-" {
        ...
        Ok(ArtifactSource::Bytes(bytes))
    } else {
        Ok(ArtifactSource::File(value.into()))
    }
}
    ...
    T: DeserializeOwned,
{
    ...
    serde_json::from_slice(&bytes).map_err(|error| {
        OffprintError::new(
            "offprint.input.json",
            ErrorStage::Validation,
            format!("{name} is not valid Offprint JSON: {error}"),
        )
    })
}
