...
#[tokio::test]
#[ignore = "requires OFFPRINT_RELEASE_BINARY and a compatible browser"]
async fn release_binary_captures_and_verifies_offline() -> TestResult {
    let binary = std::env::var_os("OFFPRINT_RELEASE_BINARY")
        .map(PathBuf::from)
        .ok_or("OFFPRINT_RELEASE_BINARY is unset")?;
    ...
    let browser_path = std::env::var_os("OFFPRINT_RELEASE_BROWSER_PATH").map(PathBuf::from);
    if browser_path.is_none() {
        ...
        return Err("OFFPRINT_RELEASE_BROWSER_PATH must name a browser executable".into());
    }
    ...
    if captured.get("captureId").and_then(Value::as_str).is_none()
        || captured
    ...
    if verified.get("method").and_then(Value::as_str) != Some("offline")
...
async fn run_owned(binary: &Path, arguments: Vec<String>, timeout: Duration) -> TestResult<Output> {
    ...
    command
        .kill_on_drop(true)
        .env_remove("OFFPRINT_RELEASE_BINARY")
        .env_remove("OFFPRINT_RELEASE_BROWSER_PATH")
        .args(arguments);
