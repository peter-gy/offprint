...
fn runner(id: &str) -> FixtureRunner {
    let (package, test_target, filter, ignored) = match id {
        ...
        "cli-pdf-output" => (
            "offprint-cli",
            None,
            "runner::tests::artifact_export_writes_and_verifies_pdf_output",
            true,
        ),
        "cli-pdf-json-output" => (
            "offprint-cli",
            None,
            "runner::tests::artifact_export_json_returns_the_committed_artifact",
            true,
        ),
        "network-denied-reopen" => (
            "offprint",
...
mod tests {
    ...
    fn each_required_fixture_has_an_observable_expectation() {
        ...
        assert!(manifest.fixtures.iter().all(|fixture| {
            !fixture.runner.package.is_empty() && !fixture.runner.filter.is_empty()
        }));
    }
