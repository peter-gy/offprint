...
use pageknot::{
    BrowserCookie, CaptureCredentials, ErrorStage, PageKnotError, RequestHeader, Result,
    SecretString,
};
...
try {
    $path = [Environment]::GetEnvironmentVariable('PAGEKNOT_CREDENTIAL_PATH', 'Process')
...
"#;
    ...
    let status = Command::new(powershell)
        ...
        .env("PAGEKNOT_CREDENTIAL_PATH", path)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
...
fn credential_error(message: impl Into<String>) -> PageKnotError {
    PageKnotError::new(
        "pageknot.input.credentials",
        ErrorStage::Validation,
        message,
