...
async fn completion_writes_script_data_to_stdout() -> TestResult {
    let cli = Cli::try_parse_from(["pageknot", "completion", "bash"])?;
    let mut input = std::io::empty();
    let mut output = Vec::new();
    let mut diagnostics = Vec::new();
    ...
    assert!(String::from_utf8_lossy(&output).contains("_pageknot"));
...
async fn missing_artifact_input_returns_a_runtime_status() -> TestResult {
    for command in ["inspect", "verify"] {
        ...
        let mut arguments = vec!["pageknot", command, path];
        if command == "verify" {
            arguments.extend(["--level", "static"]);
        }
        let cli = Cli::try_parse_from(arguments)?;
        let mut input = std::io::empty();
        let mut output = Vec::new();
        ...
        assert!(
            String::from_utf8_lossy(&diagnostics).contains("pageknot.artifact.read"),
            "{command}: {}",
            String::from_utf8_lossy(&diagnostics)
        );
    }
...
async fn missing_exported_artifact_returns_a_runtime_status() -> TestResult {
    ...
    let path = path
        .to_str()
        .ok_or_else(|| std::io::Error::other("artifact path is not UTF-8"))?;
    let cli = Cli::try_parse_from(["pageknot", "verify", path, "--format", "pdf"])?;
    let mut input = std::io::empty();
    let mut output = Vec::new();
    let mut diagnostics = Vec::new();
    ...
    assert!(output.is_empty());
    assert!(String::from_utf8_lossy(&diagnostics).contains("pageknot.artifact.read"));
    Ok(())
}

#[tokio::test]
async fn malformed_inspection_input_returns_a_runtime_status() -> TestResult {
    let cli = Cli::try_parse_from(["pageknot", "inspect", "-"])?;
    ...
    assert_eq!(exit, CommandExit::RuntimeFailure);
    assert!(output.is_empty());
    assert!(String::from_utf8_lossy(&diagnostics).contains("pageknot.verification.manifest"));
    Ok(())
}

#[tokio::test]
async fn inspect_human_output_renders_the_manifest_contract() -> TestResult {
    ...
    for field in [
        "format: Html v1 (schema 1)",
        "source: https://example.com/",
        "captured: 2026-07-27T12:00:00+00:00",
        "generator: PageKnot 0.1.0",
        ...
        "verification: Offline, passed=true, 0 network requests",
    ] {
...
async fn verify_human_output_names_the_artifact() -> TestResult {
    ...
    let path = path
        .to_str()
        .ok_or_else(|| std::io::Error::other("artifact path is not UTF-8"))?;
    let cli = Cli::try_parse_from(["pageknot", "verify", path, "--level", "static"])?;
    let mut input = std::io::empty();
    let mut output = Vec::new();
    let mut diagnostics = Vec::new();
...
async fn doctor_reports_browser_flags_as_effective_configuration() -> TestResult {
    ...
    let cli = Cli::try_parse_from(["pageknot", "doctor", "--browser-path", path, "--json"])?;
    let mut input = std::io::empty();
    let mut output = Vec::new();
    let mut diagnostics = Vec::new();
...
async fn capture_staging_failure_returns_runtime_status_with_sanitized_diagnostics() -> TestResult {
    ...
    assert_eq!(exit, CommandExit::RuntimeFailure);
    assert!(output.is_empty());
    let human = String::from_utf8(diagnostics)?;
...
async fn verify_failure_writes_sanitized_diagnostics() -> TestResult {
    ...
    assert_eq!(payload["verificationLevel"], "static");
    assert_eq!(payload["failure"]["code"], "pageknot.verification.manifest");
