...
use offprint_model::{
    BrowserSpec, CaptureEvent, CaptureId, CaptureOutput, CaptureRequest, CaptureStatus,
    ContentDigest, ErrorStage, OffprintError, PortablePath, RedactedUrl, RedactionPolicy, Result,
    UserAgentPolicy,
};
...
fn sanitized_request(request: &CaptureRequest) -> Value {
    ...
    let (artifact_target, conflict) = match request.output {
        CaptureOutput::File { conflict, .. } => (json!({"kind": "file"}), Some(conflict)),
        CaptureOutput::Memory { max_bytes } => {
            (json!({"kind": "memory", "maxBytes": max_bytes}), None)
        }
    };
    ...
    let mut value = json!({
        ...
        "capture": {
            "missingResources": request.content.missing_resources,
            "preservePasswordValues": request.content.preserve_password_values,
            "scope": request.content.scope,
            "optimizations": request.content.optimizations,
            "allowedFileRootCount": request.content.allowed_file_roots.len(),
        },
        "credentials": {
            "headerNames": request
                .credentials
...
fn sanitized_error(error: &OffprintError) -> Value {
...
fn diagnostic_error(message: impl Into<String>) -> OffprintError {
    OffprintError::new("offprint.output.diagnostics", ErrorStage::Commit, message)
}
    ...
    async fn diagnostic_bundle_excludes_secret_values_and_failure_messages() -> Result<()> {
        ...
        let warning = CaptureEvent::Warning {
            ...
            warning: CaptureWarning {
                code: "offprint.resource.fixture".to_owned(),
        ...
        assert!(bundle.contains("offprint.resource.fixture"));
        Ok(())
    }
}
