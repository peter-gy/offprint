...
pub(super) async fn load_or_create_manifest(
    ...
    {
        return Err(scheduler_error(
            "offprint.scheduler.manifest_mismatch",
            ErrorStage::Validation,
...
pub(super) fn validate_batch_manifest(
    ...
    for (job, digest) in request.jobs.iter().zip(request_digests) {
        let CaptureOutput::File { path, .. } = &job.request.output else {
            return Err(scheduler_error(
    ...
    match outcome {
        ScheduledCaptureOutcome::Succeeded {
            ...
            let (artifact_path, artifact_sha256) = match &result.artifact {
                CaptureArtifact::File { path, sha256, .. } => (Some(path.clone()), Some(*sha256)),
                CaptureArtifact::Bytes { sha256, .. } => (None, Some(*sha256)),
            };
            ResumeJobRecord {
...
fn manifest_shape_error(message: impl Into<String>) -> offprint_model::OffprintError {
    scheduler_error(
...
mod tests {
    use offprint_model::{
        BatchJob, CaptureId, CaptureRequest, CrawlRequest, OffprintError, ResumeOptions,
    };
    ...
    fn succeeded_record_requires_capture_and_file_artifact_identity() {
        ...
        assert_eq!(
            ...
            Err("offprint.scheduler.manifest_shape")
        );
    ...
    fn failed_record_requires_error_and_no_artifact_identity() {
        ...
        record.error = Some(OffprintError::new(
