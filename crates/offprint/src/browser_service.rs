...
use offprint_model::{
    ...
    ManagedBrowserState, NetworkPolicySummary, OffprintError, OutputCapability, RecoveryAction,
    Result,
};
...
impl BrowserService {
    ...
    async fn install_browser(&self, request: BrowserInstallRequest) -> Result<BrowserInfo> {
    ...
    /// Installs a managed browser and returns its operation record.
    pub async fn install(&self, request: BrowserInstallRequest) -> Result<BrowserOperationResult> {
        let browser = self.install_browser(request).await?;
    ...
    /// Lists selected and compatible browser candidates.
    async fn list_candidates(&self) -> Result<Vec<BrowserCandidate>> {
        self.state.ensure_open()?;
        let mut discovery = self.state.discovery.discover().await;
        let manager = ManagedBrowserManager::new(self.state.cache_dir.clone());
        for browser in manager.installed().await? {
            let present = discovery
                .candidates
                .iter()
                .any(|candidate| candidate.browser == browser);
            if !present {
                discovery.candidates.push(BrowserCandidate {
                    browser,
                    state: BrowserCandidateState::Compatible,
                    reason_code: "offprint.browser.compatible".to_owned(),
                    priority: u32::try_from(discovery.candidates.len()).unwrap_or(u32::MAX),
                    active_leases: 0,
                });
            }
        }
    ...
    pub async fn remove(&self, revision: &str, force: bool) -> Result<BrowserOperationResult> {
        ...
        let target = manager
            ...
            .await?
            .into_iter()
            .find(|browser| browser.revision.as_deref() == Some(revision))
            .ok_or_else(|| {
                OffprintError::new(
        ...
        if selected
            && !discovery
                .candidates
                .iter()
                .any(|candidate| candidate.browser != target)
        {
            return Err(OffprintError::new(
    ...
    /// Lists browser candidates and returns their operation record.
    pub async fn list(&self) -> Result<BrowserOperationResult> {
        let candidates = self.list_candidates().await?;
        let browser = candidates
            .iter()
            .find(|candidate| candidate.state == offprint_model::BrowserCandidateState::Selected)
    ...
    pub async fn doctor(&self) -> BrowserDoctorReport {
        ...
        let remote_failure = match self.state.default_browser() {
            BrowserSpec::Remote(endpoint) if !closed => {
                for (index, candidate) in discovery.candidates.iter_mut().enumerate() {
                    ...
                    candidate.reason_code = "offprint.browser.shadowed_by_remote".to_owned();
        ...
        let mut recovery = if closed {
            vec![RecoveryAction {
                code: "offprint.runtime.closed".to_owned(),
                description: "Construct a new Offprint handle.".to_owned(),
                command: String::new(),
                arguments: Vec::new(),
            }]
            ...
            vec![RecoveryAction {
                code: "offprint.browser.install".to_owned(),
                description: "Install the managed Chromium build.".to_owned(),
                command: "offprint".to_owned(),
...
fn capability_check(handshake: &CollectorHandshake) -> (CapabilityCheck, Option<OffprintError>) {
...
fn probe_recovery(browser: &BrowserInfo, error: &OffprintError) -> RecoveryAction {
    if browser.source != BrowserSource::Remote
        && error.code.as_str().starts_with("offprint.browser.")
    {
    ...
    let recovery = if browser.source == BrowserSource::Remote {
        ...
        " Reinstall Offprint from one release and run doctor again."
    };
    RecoveryAction {
        code: error.code.to_string(),
...
fn selected_browser_constraints(browser: &BrowserInfo) -> Vec<RecoveryAction> {
    ...
    vec![
        RecoveryAction {
            code: "offprint.input.remote_network_policy".to_owned(),
            description: "Remote CDP supports only the unrestricted network policy because the endpoint does not declare trusted proxy enforcement. Use an Offprint-owned browser when capture requires address policy enforcement.".to_owned(),
            command: String::new(),
            arguments: Vec::new(),
        },
        RecoveryAction {
            code: "offprint.browser.offline_verifier_unavailable".to_owned(),
            description: "Browser-backed offline verification requires an Offprint-owned browser because the remote endpoint does not declare trusted proxy enforcement.".to_owned(),
            command: String::new(),
            arguments: Vec::new(),
        },
    ]
...
mod tests {
    ...
    use offprint_model::{BrowserInfo, BrowserProduct, BrowserSource, CaptureId, ContentDigest};
    use offprint_protocol::{CollectorCapability, CollectorHandshake, ProtocolVersion};
    ...
    fn incompatible_handshake_reports_the_peer_version() {
        ...
        assert_eq!(
            ...
            Some("offprint.collector.protocol_major")
        );
    }
    ...
    fn capability_gap_is_named_in_the_doctor_record() {
        ...
        assert_eq!(
            failure.as_ref().map(|error| error.code.as_str()),
            Some("offprint.collector.capability")
        );
    }
