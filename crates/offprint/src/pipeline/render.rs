...
use offprint_browser::{LoadedResource, NetworkGuard, PageSession};
use offprint_capture::{CaptureBudget, StoredContent};
...
use offprint_model::{
    ...
    MissingResourcePolicy, OffprintError, ResourceError, ResourceId, ResourceOutcome,
...
use super::resource_materialization::{
    LoadedResourceData, ResourceLoadContext, ResourceMaterializer, data_url, is_css, is_html,
    is_svg, resource_record, url_without_fragment, validate_file_resource, validate_network_url,
};
...
struct ResourceResolver<'a> {
    ...
    session_id: &'a str,
    browser_frame_id: &'a str,
    manifest_frame_id: offprint_model::FrameId,
...
impl ResourceLoadKey {
    fn new(session_id: &str, browser_frame_id: &str, url: &Url) -> Self {
        Self {
            session_id: session_id.to_owned(),
            browser_frame_id: browser_frame_id.to_owned(),
...
pub(super) struct ResourceState {
    ...
    failed_loads: BTreeMap<ResourceLoadKey, OffprintError>,
...
async fn prefetch_resources(
    ...
    batch: &[(&offprint_document::DiscoveredDocumentResource, ResourceId)],
    failed_loads: &BTreeMap<ResourceLoadKey, OffprintError>,
    ...
    let candidates = batch
        ...
        .filter_map(|(resource, resource_id)| {
            if matches!(
                ...
                let key = ResourceLoadKey::new(context.session_id, context.browser_frame_id, &url);
    ...
    futures_util::stream::iter(candidates)
        .map(|(resource_id, url)| async move {
            let validation = match url.scheme() {
                ...
                "file" => {
                    validate_file_resource(&url, &context.request.content.allowed_file_roots).await
                }
                "blob" => Ok(()),
                _ => Ok(()),
            };
    ...
    Box::pin(async move {
        ...
        for batch in references.chunks(concurrency) {
            ...
            let mut resolver = ResourceResolver {
                ...
                browser_frame_id: context.browser_frame_id,
                manifest_frame_id,
                guard: context.guard,
...
fn apply_inline_frame_replacements(
    ...
    inline_frames: &[offprint_document::InlineFrame],
    replacements: &BTreeMap<offprint_model::NodeId, InlineFrameReplacement>,
) -> Result<()> {
...
impl ResourceResolver<'_> {
    ...
    fn resolve<'a>(
        ...
        Box::pin(async move {
            ...
            let load_key = ResourceLoadKey::new(self.session_id, self.browser_frame_id, &url);
            ...
            if is_css(&media_type) {
                ...
                let transformed = async {
                    let css = String::from_utf8(bytes).map_err(|error| {
                        OffprintError::new(
                            "offprint.resource.css_encoding",
                            ErrorStage::Resource,
                    ...
                    if css_resources.parse_mode() == CssParseMode::PreservationFallback {
                        let warning = CaptureWarning {
                            code: "offprint.resource.css_preservation".to_owned(),
                ...
                bytes = match transformed {
                    ...
                    Err(error) if error.code.as_str() == "offprint.resource.limit" => {
                        return Err(error);
                    }
            ...
            if is_svg(&media_type) {
                ...
                let transformed = async {
                    ...
                    discovered.rewrite(&mut document, &replacements)?;
                    offprint_document::serialize_subtree(&document, svg_root).map_err(|error| {
                        OffprintError::new(
                            "offprint.resource.svg_serialize",
                            ErrorStage::Resource,
            ...
            if is_html(&media_type) {
                if nested_depth >= self.request.limits.frame_depth {
                    let error = OffprintError::new(
                        "offprint.resource.html_depth",
                        ErrorStage::Resource,
                        "embedded HTML depth exceeds the configured frame limit",
                    );
                    return self.missing(url, role, resource_id, error);
                }
                let key = url_without_fragment(&url).to_string();
                if !self.active_html.insert(key.clone()) {
                    let error = OffprintError::new(
                        "offprint.resource.html_cycle",
                        ErrorStage::Resource,
                        "embedded HTML resources contain a cycle",
                    );
                    return self.missing(url, role, resource_id, error);
                }
                let transformed = async {
                    let resource_inputs = {
                        let document = Document::parse(&bytes);
                        if !document.inline_frames().is_empty() {
                            return Err(OffprintError::new(
                                "offprint.resource.html_frame",
                                ErrorStage::Resource,
                                "embedded HTML resources cannot contain nested frames",
                            ));
                        }
                        let remaining = self.remaining_resources()?;
                        discover_document_resources_bounded(&document, &url, remaining)?
                    ...
                    if discovered.resources().len() != resource_inputs.len() {
                        return Err(OffprintError::new(
                            "offprint.resource.rewrite",
                            ErrorStage::Resource,
                            "embedded HTML resource inventory changed before rewriting",
                        ));
                    }
                    discovered.rewrite(&mut document, &replacements)?;
                    sanitize_safe_static(&mut document);
                    ensure_render_freeze_styles(&mut document)?;
                    serialize_document(&document).map_err(|error| {
                        OffprintError::new(
                            "offprint.resource.html_serialize",
                            ErrorStage::Resource,
                            format!("embedded HTML resource could not be serialized: {error}"),
                        )
                    })
                }
                .await;
                self.active_html.remove(&key);
                bytes = match transformed {
                    Ok(bytes) => {
                        stored_content = None;
                        bytes
    ...
    async fn load(
        ...
        ResourceLoadContext {
            ...
            browser_frame_id: self.browser_frame_id,
            guard: self.guard,
    ...
    fn missing(
        ...
        self.progress();
        if self.request.content.missing_resources == MissingResourcePolicy::Fail {
            return Err(error);
        }
        let warning = CaptureWarning {
        ...
        self.state.warnings.push(warning);
        Ok(offprint_html::empty_resource_data_url(role))
    }
...
#[cfg(test)]
mod tests {
    use offprint_capture::CaptureBudget;
    use offprint_model::CaptureLimits;

    use super::ResourceState;

    #[test]
    fn resource_capacity_fails_before_growing_the_inventory() -> offprint_model::Result<()> {
