...
use pageknot_model::{ContentDigest, ErrorStage, PageKnotError, Result};
        ...
        S: Stream<Item = Result<Bytes>> + Send,
    {
        ...
        while let Some(chunk) = stream.next().await {
            ...
            resource_bytes = resource_bytes.checked_add(chunk_bytes).ok_or_else(|| {
                content_error("pageknot.resource.limit", "resource byte count overflowed")
            })?;
...
fn content_error(code: &'static str, message: impl Into<String>) -> PageKnotError {
    PageKnotError::new(code, ErrorStage::Resource, message)
}
