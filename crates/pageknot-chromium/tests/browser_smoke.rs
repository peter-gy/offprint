...
use pageknot_chromium::{
    ChromiumDiscovery, ChromiumLaunchOptions, ChromiumPage, ChromiumProcess, CollectorLimits,
    ResourceObservationLimits, collect_frame_observation, collect_page_observation,
};
use pageknot_model::{
    BrowserEnvironment, CaptureId, CapturePolicy, CaptureScope, FrameId, NetworkPolicy,
...
async fn local_browser_navigates_collects_and_closes() -> TestResult {
    ...
    server
        .register(
            ...
            FixtureResponse::html(
                r#"<!doctype html><title>PageKnot fixture</title>
    ...
    .await?;
    assert_eq!(observation.title, "PageKnot fixture");
...
async fn network_idle_waits_for_finite_asset_requests() -> TestResult {
    ...
    let readiness = page
        .settle(&ReadinessPolicy {
            ...
            network_quiet: pageknot_model::Milliseconds::new(50),
...
async fn collector_uses_document_start_primordials_after_page_poisoning() -> TestResult {
    ...
    page.evaluate(
        r#"(() => {
          ...
          try {
            globalThis.__pageknotCollector = {
...
async fn collector_reads_page_controlled_shadow_mode_once() -> TestResult {
    ...
    assert!(
        observation
            .html
            .contains(r#"data-pageknot-shadow-mode="closed""#)
    );
    page.close().await?;
    process.close().await?;
    server.close().await;
...
async fn structural_repair_uses_captured_attribute_writes() -> TestResult {
    ...
    assert!(observation.html.contains("pageknot-repair-data"));
    assert!(
        observation
            .html
            .contains("application/vnd.pageknot.repair+json")
    );
    page.close().await?;
    process.close().await?;
    server.close().await;
...
async fn freeze_orchestration_uses_document_start_primordials() -> TestResult {
    ...
    assert!(observation.html.contains("data-pageknot-freeze"));
    page.close().await?;
    process.close().await?;
    server.close().await;
    Ok(())
...
async fn capture_delay_uses_a_browser_host_timer() -> TestResult {
    ...
    let readiness = page
        .settle(&ReadinessPolicy {
            mode: ReadinessMode::Load,
            delay: pageknot_model::Milliseconds::new(50),
            ..ReadinessPolicy::default()
        })
        .await?;
...
async fn capture_contexts_deny_download_output_in_headless_and_headed_modes() -> TestResult {
    const DOWNLOAD_NAME: &str = "pageknot-context-download.bin";
...
async fn observed_response_honors_a_lower_per_resource_limit() -> TestResult {
    ...
    assert_eq!(error.code.as_str(), "pageknot.resource.limit", "{error:?}");
...
async fn collector_materializes_motion_and_shadow_state_at_the_freeze_boundary() -> TestResult {
    ...
    assert!(
        observation.html.contains(r#"data-pageknot-scroll-y="240""#),
        "{}",
        observation.html
    );
    assert!(
        observation
            .html
            .contains(r#"data-pageknot-scroll-left="30""#),
        "{}",
        observation.html
    );
    assert!(
        observation
            .html
            .contains(r#"data-pageknot-scroll-top="70""#),
        "{}",
        observation.html
    );
    assert!(
    ...
    assert!(
        observation
            .html
            .matches("data-pageknot-scroll-left")
            .count()
            >= 3
            && observation.html.matches("data-pageknot-scroll-top").count() >= 3,
        "{}",
        observation.html
    );
    assert!(
    ...
    let worker_state = tokio::time::timeout(Duration::from_secs(2), async {
        loop {
            ...
            if state.as_str() == Some("nested-started,outer-started") {
                return Ok::<Value, pageknot_model::PageKnotError>(state);
            }
...
async fn collector_captures_the_first_selector_match_and_reports_selector_errors() -> TestResult {
    ...
    assert_eq!(
        ...
        Err("pageknot.selector.not_found")
    );
...
async fn collector_maps_frame_owners_after_reserved_ancestor_cleanup() -> TestResult {
    ...
    parent
        .register(
            "/",
            FixtureResponse::html(format!(
                r#"<section id="pageknot-state-script">
                  <iframe src="{reserved_id}"></iframe>
                </section>
                <section data-pageknot-animation-style>
...
async fn local_executable() -> TestResult<pageknot_model::PortablePath> {
