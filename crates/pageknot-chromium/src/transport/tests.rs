...
async fn actor_send_timeout_fails_the_pending_command() -> TestResult {
    ...
    assert_eq!(
        ...
        Some("pageknot.browser.cdp_transport")
    );
    assert_eq!(
...
fn test_client(
    ...
    task: Option<JoinHandle<pageknot_model::Result<()>>>,
...
async fn spawn_pending_task() -> (JoinHandle<pageknot_model::Result<()>>, Arc<AtomicBool>) {
    ...
    let task = tokio::spawn(async move {
        ...
        pending::<pageknot_model::Result<()>>().await
    });
