...
use pageknot_model::{
    BrowserInfo, BrowserSource, ErrorStage, ManagedBrowserState, PageKnotError, Result,
};
...
impl ManagedBrowserManager {
    ...
    pub async fn install(&self, revision: Option<&str>) -> Result<BrowserInfo> {
        ...
        if final_dir.exists() {
            ...
            .with_detail(
                "recoveryCommand",
                format!("pageknot browser remove {} --force", entry.revision),
            ));
        }

        let archive = download::download(&self.cache_dir, entry).await?;
        let staging = tempfile::Builder::new()
            .prefix(".pageknot-browser-stage-")
...
fn managed_error(code: &'static str, message: impl Into<String>) -> PageKnotError {
