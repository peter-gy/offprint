...
use pageknot_browser::LoadedResource;
use pageknot_model::{ErrorStage, PageKnotError, ResourceRetrievalSource, Result};
...
impl ChromiumPage {
    ...
    pub async fn load_resource_in_session(
        ...
        let resource = response.get("resource").ok_or_else(|| {
            with_resource_url(
                PageKnotError::new(
                    "pageknot.resource.protocol_shape",
                    ErrorStage::Resource,
            ...
            .unwrap_or(false)
        {
            ...
            return Err(with_resource_url(
                PageKnotError::new("pageknot.resource.load", ErrorStage::Resource, reason)
                    .retryable(true),
                url,
            ));
        }
