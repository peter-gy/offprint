...
use pageknot_browser::{NavigationRedirect, NavigationResult};
            ...
            loop {
                let event = tokio::select! {
                    ...
                    event = events.recv() => event.map_err(|error| match error {
                        broadcast::error::RecvError::Lagged(skipped) => PageKnotError::new(
                            "pageknot.browser.cdp_event_lag",
                            ErrorStage::Navigation,
                            format!("navigation event consumer fell behind by {skipped} events"),
                        )
                        .retryable(true),
                        broadcast::error::RecvError::Closed => PageKnotError::new(
                        ...
                        if let Some(redirect) = event.params.get("redirectResponse") {
                                ...
                                > usize::try_from(redirect_limit).unwrap_or(usize::MAX)
                            {
                                let _ignored = self
                                    ...
                                    .await;
                                return Err(PageKnotError::new(
                                    "pageknot.navigation.redirect_limit",
                                    ErrorStage::Navigation,
        ...
        loop {
                ...
                .await
            {
                ...
                Err(error)
                    if matches!(
                        error.code.as_str(),
                        "pageknot.browser.cdp_timeout" | "pageknot.browser.cdp_command"
                    ) =>
                {
                    false
                }
