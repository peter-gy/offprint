...
use offprint_model::{ErrorStage, FrameId, NodeId, OffprintError, Result};
...
impl Document {
    ...
    fn embed_frame_with_id(
        ...
        if let Some(frame_id) = frame_id {
            attrs.push(Attribute {
                name: QualName::new(None, ns!(), "data-offprint-frame-id".into()),
                ...
                let base_url = attrs
                    ...
                    .find(|attribute| {
                        attribute.name.ns == ns!()
                            && attribute.name.local.as_ref() == "data-offprint-frame-base"
                    })
                    .map(|attribute| attribute.value.to_string());
                let captured = attrs.iter().any(|attribute| {
                    attribute.name.ns == ns!()
                        && attribute.name.local.as_ref() == "data-offprint-frame-id"
                });
                Some(InlineFrame {
                    node_id,
                    html,
    ...
    pub fn resolve_visual_fallback(&mut self, id: &str, data_url: &str) -> Result<()> {
        if self.resolve_visual_fallback_at_depth(id, data_url, 0)? {
            ...
            Err(OffprintError::new(
                "offprint.transform.visual_fallback",
                ErrorStage::Transform,
    ...
    fn resolve_visual_fallback_at_depth(
        ...
        for node in &mut self.nodes {
            ...
            if !attrs.iter().any(|attribute| {
                attribute.name.ns == ns!()
                    && attribute.name.local.as_ref() == "data-offprint-visual-fallback"
            ...
            attrs.retain(|attribute| {
                attribute.name.ns != ns!()
                    || !matches!(
                        ...
                        "data-offprint-visual-fallback" | "src"
                    )
            });
            attrs.push(Attribute {
