...
const FREEZE_ATTRIBUTE: &str = "data-pageknot-freeze";
...
fn has_direct_freeze_style(document: &Document, parent: pageknot_model::NodeId) -> bool {
    document
        .node(parent)
        .into_iter()
...
fn append_freeze_style(document: &mut Document, parent: pageknot_model::NodeId) -> Result<()> {
...
mod tests {
    ...
    fn safe_static_sanitization_removes_active_content_and_preserves_json_metadata() {
        let mut document = Document::parse(
            br#"<html><head>
                ...
                <script type="application/ld+json">{"name":"PageKnot"}</script>
    ...
    fn rendering_freeze_rules_cover_the_document_and_declarative_shadow_roots() {
        ...
        assert_eq!(
            html.as_deref()
                .map(|html| html.matches("data-pageknot-freeze").count()),
            Some(2)
        );
    }
}
