...
use pageknot_model::{CaptureId, ContentDigest, ErrorStage, FrameId, PageKnotError, Result};
...
#[cfg(test)]
mod tests {
    use pageknot_model::{CaptureId, ContentDigest, FrameId};
    ...
    fn plan(
        ...
        payload: &[u8],
        maximum_chunk_bytes: u64,
    ) -> Result<TransferPlan, pageknot_model::PageKnotError> {
        TransferPlan::new(
    ...
    fn plan_binds_chunk_count_to_encoded_bytes_and_negotiated_size() {
        ...
        assert!(plan(&capture_id, frame_id, 3, b"pageknot", 3).is_ok());
        assert_eq!(
            plan(&capture_id, frame_id, 4, b"pageknot", 3)
                .as_ref()
                .map_err(|error| error.code.as_str()),
            Err("pageknot.collector.chunk_total")
        );
    }
    ...
    fn plan_rejects_payloads_above_the_host_limit() {
        let capture_id = CaptureId::new();
        let frame_id = FrameId::new(1);
        let descriptor = descriptor(capture_id.clone(), frame_id, 1, b"pageknot");
    ...
    -> Result<(), pageknot_model::PageKnotError> {
        let capture_id = CaptureId::new();
        let frame_id = FrameId::new(1);
        let transfer = plan(&capture_id, frame_id, 2, b"pageknot", 4)?;
    ...
    #[test]
    fn assembler_rejects_an_empty_intermediate_chunk() -> Result<(), pageknot_model::PageKnotError>
    {
        let capture_id = CaptureId::new();
        let frame_id = FrameId::new(1);
        let transfer = plan(&capture_id, frame_id, 2, b"pageknot", 4)?;
        ...
        assert_eq!(
            result.as_ref().map_err(|error| error.code.as_str()),
            Err("pageknot.collector.chunk_length")
        );
        Ok(())
    }
    ...
    -> Result<(), pageknot_model::PageKnotError> {
        let capture_id = CaptureId::new();
        let frame_id = FrameId::new(1);
        let transfer = plan(&capture_id, frame_id, 1, b"pageknot", 8)?;
        let mut assembler = ChunkAssembler::new(transfer);
        let result = assembler.push(ChunkEnvelope::new(
            capture_id,
            frame_id,
        ...
        assert_eq!(
            result.as_ref().map_err(|error| error.code.as_str()),
            Err("pageknot.collector.payload_checksum")
        );
        Ok(())
    }
