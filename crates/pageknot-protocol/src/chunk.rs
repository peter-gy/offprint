...
mod tests {
    ...
    fn envelope_validates_its_declared_length_and_checksum() {
        ...
        let mut envelope = ChunkEnvelope::new(capture_id, frame_id, 0, 1, b"pageknot".to_vec());
        ...
        assert_eq!(
            ...
            Err("pageknot.collector.chunk_checksum")
        );
    }
    ...
    fn envelope_rejects_bytes_above_the_negotiated_limit() {
        let capture_id = CaptureId::new();
        let frame_id = FrameId::new(1);
        let envelope = ChunkEnvelope::new(capture_id, frame_id, 0, 1, b"pageknot".to_vec());
        let result = envelope.validate(7);

        assert_eq!(
            result.as_ref().map_err(|error| error.code.as_str()),
            Err("pageknot.collector.chunk_limit")
        );
    }
}
