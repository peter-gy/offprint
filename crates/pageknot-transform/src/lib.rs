...
pub fn transform_parsed_document(mut document: Document) -> Result<TransformedDocument> {
...
fn transform_document_tree(document: &mut Document, depth: u16) -> Result<bool> {
    ...
    structural_repair |= pageknot_html::apply_structural_repair(document)?;
...
pub fn encode_verified_artifact(
    transformed: &TransformedDocument,
    manifest: ArtifactManifest,
) -> Result<pageknot_html::VerifiedHtml<Vec<u8>>> {
    let mut bytes = Vec::new();
    let manifest = encode_artifact_to(transformed, manifest, &mut bytes)?;
    let verified = pageknot_html::verify_html(bytes)?;
...
pub fn encode_artifact_to(
    ...
    pageknot_html::encode_html_to(transformed.document(), &manifest, output)?;
    Ok(manifest)
}
...
mod tests {
    ...
    use pageknot_model::{ArtifactManifest, ContentDigest, StructuralRepair};
    ...
    fn structural_repair_state_replaces_stale_false_manifest_state() -> TestResult {
        let source = format!(
            ...
            pageknot_html::REPAIR_DATA_ELEMENT_ID,
            pageknot_html::REPAIR_MEDIA_TYPE,
        );
    ...
    fn nested_inline_frames_are_transformed_before_encoding() -> TestResult {
        let artifact = transform_artifact(
            br#"<html><head></head><body>
                <iframe data-pageknot-frame-id="2" srcdoc="<html><head></head><body><script>active()</script><p>frame</p></body></html>"></iframe>
                </body></html>"#,
            {
        ...
        assert_eq!(output.matches("data-pageknot-freeze").count(), 2);
        assert!(artifact.verification.passed);
        Ok(())
    }
