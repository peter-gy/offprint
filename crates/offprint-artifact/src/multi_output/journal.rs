...
use offprint_model::{
    ArtifactFormat, ContentDigest, ErrorStage, FormatVerification, OffprintError, Result,
};
...
pub(super) struct JournalEntry {
    ...
    pub(super) verification: FormatVerification,
...
impl TransactionNamespace {
    pub(super) fn acquire(output_directory: &Path) -> Result<Self> {
        ...
        let path = parent.join(format!(".offprint-transactions-{identity}"));
        match fs::symlink_metadata(&path) {
            Ok(metadata) if metadata.file_type().is_symlink() || !metadata.is_dir() => {
                return Err(journal_error(
                    ErrorStage::Validation,
                    "Offprint transaction namespace must be a directly addressed directory",
                ));
            }
            Ok(_) => {}
            ...
            Err(error) => {
                return Err(journal_io_error(
                    ErrorStage::Validation,
                    "failed to inspect the Offprint transaction namespace",
                    error,
                ));
            }
        }
        ...
        lock.try_lock_exclusive().map_err(|error| {
            journal_error(
                ErrorStage::Commit,
                "another Offprint export transaction owns this output directory",
            )
            .retryable(true)
    ...
    pub(super) fn transaction_roots(&self) -> Result<Vec<PathBuf>> {
        ...
        for entry in fs::read_dir(&self.path).map_err(|error| {
            journal_io_error(
                ErrorStage::Commit,
                "failed to enumerate the Offprint transaction namespace",
                error,
            )
        })? {
                ...
                || !metadata.is_dir()
            {
                return Err(journal_error(
                    ErrorStage::Commit,
                    "Offprint transaction namespace contains an unrecognized entry",
                )
...
fn validate_entries(entries: &[JournalEntry]) -> Result<()> {
    if entries.is_empty() || entries.len() > MAXIMUM_JOURNAL_VARIANTS {
        return Err(journal_error(
            ErrorStage::Commit,
            "export transaction journal has an invalid format count",
        ));
    }
...
fn journal_error(stage: ErrorStage, message: impl Into<String>) -> OffprintError {
