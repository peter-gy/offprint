...
use offprint_model::{
    ...
    CaptureId, ContentPolicy, ErrorStage, FrameId, Milliseconds, NetworkPolicy, OffprintError,
    ReadinessMode, ReadinessPolicy, RequestHeader, ResourceRetrievalSource, Result,
};
use tokio_util::sync::CancellationToken;
use url::Url;

use crate::{FrameObservation, NetworkGuard, VisualFallback};
...
pub struct ObservationLimits {
    /// Depth of the addressed frame.
    pub frame_depth: u16,
    /// Maximum encoded bytes in one observation transfer chunk.
...
pub struct ObservedFrame {
    /// Provider-neutral rendered state for the frame.
...
#[async_trait]
/// Acquires browser leases for Offprint capture jobs.
...
#[async_trait]
/// Browser operations required by the Offprint capture pipeline.
pub trait PageSession: fmt::Debug + Send + Sync {
    ...
    /// Observes bounded rendered state in one frame.
    ...
    async fn print_to_pdf(
        ...
        Err(OffprintError::new(
            "offprint.browser.pdf_unavailable",
            ErrorStage::Encoding,
