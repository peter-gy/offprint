use pageknot::{
    ArtifactResult, BrowserChannel, BrowserSource, CaptureRequest, CaptureTerminalStatus,
    ErrorStage, NetworkPolicy, PageKnot, PageKnotError, Result, VerificationPolicy,
};
...
async fn doctor_rejects_a_discoverable_browser_that_cannot_launch() -> Result<()> {
    ...
    let doctor = pageknot.browsers().doctor().await;
    ...
    assert!(
        doctor
            .recovery
            .iter()
            .any(|action| action.code.starts_with("pageknot.browser.launch"))
    );
...
async fn remote_doctor_reports_constraints_and_static_capture_keeps_process_ownership() -> Result<()>
{
    ...
    let pageknot = PageKnot::builder().cdp_url(remote_endpoint).build()?;
    let doctor = pageknot.browsers().doctor().await;
    ...
    assert!(
        doctor
            .recovery
            .iter()
            .any(|action| action.code == "pageknot.input.remote_network_policy")
    );
    assert!(
        doctor
            .recovery
            .iter()
            .any(|action| action.code == "pageknot.browser.offline_verifier_unavailable")
    );
...
async fn remote_doctor_reports_endpoint_failure_without_falling_back_locally() -> Result<()> {
    ...
    assert!(
        doctor
            .recovery
            .iter()
            .any(|action| action.code == "pageknot.browser.cdp_connect")
    );
    assert!(
        doctor
            .recovery
...
#[cfg(unix)]
fn fixture_io_error(error: std::io::Error) -> PageKnotError {
    PageKnotError::new(
        "pageknot.internal.fixture",
        ErrorStage::Internal,
