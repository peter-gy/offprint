...
use pageknot::{
    ArtifactResult, ArtifactSpec, BrowserEnvironment, CaptureRequest, Clock, NetworkPolicy,
    PageKnot, ReadinessMode, ResourceOutcome, ResourceRetrievalSource,
};
...
async fn tainted_canvas_uses_a_clipped_browser_fallback() -> TestResult {
    ...
    request.artifact = ArtifactSpec::html_bytes(4 * 1024 * 1024);
    ...
    let manifest = pageknot_html::inspect_html(content)?;
    ...
    assert!(result.warnings.iter().all(|warning| {
        !matches!(
            warning.code.as_str(),
            "pageknot.canvas.capture_failed" | "pageknot.canvas.capture_unavailable"
        )
    }));
    assert!(matches!(
    ...
    assert!(html.contains("data-pageknot-canvas"), "{html}");
    ...
    server
        .register(
            ...
            FixtureResponse::html(
                ...
                <p>data-pageknot-visual-fallback="0"</p>
                <canvas id="plot" width="64" height="64" style="display:block"></canvas>
                <script>
                const gl = document.getElementById("plot").getContext("webgl");
                gl.clearColor(0.1, 0.6, 0.3, 1);
...
async fn malformed_browser_dom_reopens_with_the_observed_tree() -> TestResult {
    ...
    let html = String::from_utf8_lossy(&content);
    assert!(html.contains("pageknot-repair-data"), "{html}");
    assert!(html.contains("pageknot-repair-script"), "{html}");
    pageknot.close().await?;
    ...
    let repaired = page
        .evaluate(
            "document.querySelector('#outer > #inner') !== null && !document.querySelector('[data-pageknot-node]')",
        )
        .await?;
...
async fn detached_oopif_returns_a_typed_frame_outcome() -> TestResult {
    ...
    assert_eq!(error.code.as_str(), "pageknot.frame.detached");
    pageknot.close().await?;
    parent_server.close().await;
    child_server.close().await;
    Ok(())
...
async fn nested_oopif_replaces_its_owner_inside_the_inline_frame() -> TestResult {
    ...
    let result = pageknot.captures().start(request).await?.wait().await?;
    let content = artifact_bytes(&result)?;
    let document = pageknot_document::Document::parse(content);
    let outer = document
        .inline_frames()
        .into_iter()
        .find(|frame| frame.html.contains("outer frame shell"))
        .ok_or("captured outer frame is missing")?;
    let nested_frames = pageknot_document::Document::parse(outer.html.as_bytes()).inline_frames();

    assert_eq!(pageknot_html::inspect_html(content)?.frames, 5);
...
async fn request_variants_keep_distinct_resource_bytes() -> TestResult {
    ...
    let result = pageknot
        ...
        .to_bytes(4 * 1024 * 1024)
        .await?;
    let manifest = pageknot_html::inspect_html(artifact_bytes(&result)?)?;
...
async fn redirect_address_policy_rejects_a_disallowed_host() -> TestResult {
    ...
    assert_eq!(error.code.as_str(), "pageknot.navigation.address_blocked");
    pageknot.close().await?;
    server.close().await;
    Ok(())
}
...
fn artifact_bytes(result: &pageknot::CaptureResult) -> TestResult<&[u8]> {
...
fn captured_png_center(content: &[u8]) -> TestResult<[u8; 4]> {
    ...
    let mut documents = vec![pageknot_document::Document::parse(content)];
    while let Some(document) = documents.pop() {
        let resources = pageknot_document::discover_document_resources(&document, &base_url)?;
        ...
        documents.extend(
            document
                .inline_frames()
                .into_iter()
                .map(|frame| pageknot_document::Document::parse(frame.html.as_bytes())),
        );
    }
