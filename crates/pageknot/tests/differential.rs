...
use pageknot::{ArtifactResult, BrowserInfo, PageKnot, PortablePath, ResourceSummary};
use pageknot_chromium::{ChromiumLaunchOptions, ChromiumProcess};
use pageknot_model::BrowserEnvironment;
...
struct DifferentialReport {
    schema_version: u32,
    pageknot_duration_milliseconds: u128,
    singlefile_duration_milliseconds: u128,
    pageknot_bytes: u64,
    singlefile_bytes: u64,
    pageknot_peak_resident_bytes: u64,
    singlefile_peak_resident_bytes: u64,
    browser: BrowserInfo,
    pageknot_resources: ResourceSummary,
    pixel_similarity: f64,
    pageknot_invariants: Value,
...
#[tokio::test]
#[ignore = "requires PAGEKNOT_SINGLEFILE_EXECUTABLE and a compatible Chromium browser"]
async fn singlefile_differential_fixture_preserves_offline_invariants() -> TestResult {
    let singlefile = std::env::var_os("PAGEKNOT_SINGLEFILE_EXECUTABLE")
        .map(PathBuf::from)
        .ok_or("PAGEKNOT_SINGLEFILE_EXECUTABLE is unset")?;
    ...
    let pageknot_path = output.path().join("pageknot.html");
    ...
    let pageknot_memory = PeakMemoryMonitor::start(process_id);
    let pageknot_started = Instant::now();
    let result = pageknot
        .capture(source.as_str())?
        .save(PortablePath::from_path_buf(pageknot_path.clone())?)
        .await;
    let pageknot_duration = pageknot_started.elapsed();
    let pageknot_peak_resident_bytes = pageknot_memory.stop().await?;
    let result = result?;
    assert!(result.verification.passed);
    assert_eq!(result.verification.network_requests, 0);
    assert!(result.resources.is_complete());
    let pageknot_resources = result.resources;
    let ArtifactResult::File {
        bytes: pageknot_bytes,
        ..
    } = result.artifact
    else {
        return Err("PageKnot differential capture returned byte output".into());
    };
    ...
    let pageknot_probe = probe_artifact(&process, &pageknot_path).await?;
    let singlefile_probe = probe_artifact(&process, &singlefile_path).await?;
    process.close().await?;
    server.close().await;

    let pageknot_common_invariants = common_invariants(&pageknot_probe.invariants);
    let singlefile_common_invariants = common_invariants(&singlefile_probe.invariants);
    let pageknot_frame_complete = frame_is_complete(&pageknot_probe.invariants);
    let singlefile_frame_complete = frame_is_complete(&singlefile_probe.invariants);
    let similarity = pixel_similarity(&pageknot_probe.screenshot, &singlefile_probe.screenshot)?;

    write_report(&DifferentialReport {
        schema_version: 1,
        pageknot_duration_milliseconds: pageknot_duration.as_millis(),
        singlefile_duration_milliseconds: singlefile_duration.as_millis(),
        pageknot_bytes,
        singlefile_bytes,
        pageknot_peak_resident_bytes,
        singlefile_peak_resident_bytes,
        browser,
        pageknot_resources,
        pixel_similarity: similarity,
        pageknot_invariants: pageknot_probe.invariants,
        singlefile_invariants: singlefile_probe.invariants,
        frame_comparison: FrameComparison {
            pageknot_complete: pageknot_frame_complete,
            singlefile_complete: singlefile_frame_complete,
        },
    })?;
    assert_eq!(pageknot_common_invariants, singlefile_common_invariants);
    assert!(pageknot_frame_complete);
    assert!(
        similarity >= 0.85,
        ...
        .evaluate(
            r##"JSON.stringify({
              ...
              canvasWidth: document.querySelector("canvas")?.width ??
                document.querySelector("img[data-pageknot-canvas]")?.width ??
                document.querySelector("img[data-pageknot-visual-fallback]")?.width
            })"##,
        )
        .await?
        .as_str()
