...
use pageknot::{ArtifactResult, BrowserInfo, PageKnot};
...
async fn repeated_capture_memory_and_process_use_remain_bounded() -> TestResult {
    ...
    for sequence in 1..=CAPTURES {
        ...
        let result = pageknot
            ...
            .to_bytes(MAXIMUM_ARTIFACT_BYTES)
            .await?;
        assert!(result.verification.passed);
        assert_eq!(result.verification.network_requests, 0);
        let ArtifactResult::Bytes { bytes, content, .. } = result.artifact else {
        ...
        if browser.is_none() {
            browser = Some(pageknot_html::inspect_html(&content)?.browser);
        }
    ...
    assert_eq!(
        ...
        "browser processes survived PageKnot close"
    );
    ...
    assert!(
        !profile_path.exists(),
        "owned Chromium profile survived PageKnot close: {}",
...
fn profile_path_from_argument(argument: &std::ffi::OsStr) -> Option<PathBuf> {
    ...
    path.file_name()
        .and_then(std::ffi::OsStr::to_str)
        .is_some_and(|name| name.starts_with("pageknot-browser-"))
        .then_some(path)
}
