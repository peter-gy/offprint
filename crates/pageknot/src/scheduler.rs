...
impl SchedulerService {
    ...
    ///
    /// A failed capture becomes one outcome and does not cancel neighboring
    /// jobs. When resume state is configured, each terminal outcome is
    /// checkpointed through an atomic file transaction.
    pub async fn batch(&self, mut request: BatchRequest) -> Result<BatchResult> {
        ...
        let mut running = stream::iter(pending.into_iter().map(|index| {
            ...
            async move {
                let outcome = match service.start(capture_request).await {
                    Ok(job) => match job.wait().await {
                        Ok(result) => ScheduledCaptureOutcome::Succeeded {
                            id,
                            request_sha256,
                            result,
                        },
                        Err(error) => ScheduledCaptureOutcome::Failed {
                            id,
                            request_sha256,
                            error,
                        },
    ...
    /// ordinal and URL digest. Link discovery is processed in frontier order,
    /// so completion timing does not affect subsequent work.
            ...
            let futures = active.iter().cloned().map(|item| {
                ...
                async move {
                    ...
                    let outcome = match request_sha256 {
                        Ok(request_sha256) => match service.start(capture_request).await {
                            Ok(job) => match job.wait().await {
                                Ok(result) => ScheduledCaptureOutcome::Succeeded {
                                    id: item.url.as_str().to_owned(),
                                    request_sha256,
                                    result,
                                },
                                Err(error) => ScheduledCaptureOutcome::Failed {
                        ...
                        Err(error) => ScheduledCaptureOutcome::Failed {
                            id: item.url.as_str().to_owned(),
                            request_sha256: ContentDigest::sha256(item.url.as_str()),
                            error,
                        },
                    };
                    (item, outcome)
...
async fn load_or_create_manifest(
    ...
    let mut manifest = match metadata {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => ResumeManifest {
            schema_version: SCHEMA_VERSION,
            kind,
            plan_sha256,
            jobs: BTreeMap::new(),
...
async fn persist_manifest_if_configured(
    options: Option<&ResumeOptions>,
...
fn record_from_outcome(outcome: &ScheduledCaptureOutcome) -> ResumeJobRecord {
    match outcome {
...
async fn resume_artifact_is_current(record: &ResumeJobRecord) -> bool {
...
async fn refresh_crawl_resume_state(
    ...
    for key in keys {
        ...
        if retry {
            if let (Some(url), Some(depth), Some(ordinal)) =
...
fn crawl_seen_urls(manifest: &ResumeManifest) -> BTreeSet<String> {
    manifest
        .jobs
        .values()
        .filter_map(|record| record.url.as_ref())
        .chain(manifest.frontier.iter().map(|item| &item.url))
...
mod tests {
    use pageknot_model::{
        BrowserSpec, CaptureRequest, CrawlRequest, NetworkPolicy, PortablePath, VerificationPolicy,
    };
    use url::Url;

    use super::validate_crawl_request;
