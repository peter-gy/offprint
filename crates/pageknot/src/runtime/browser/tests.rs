...
impl BrowserLease for CountingLease {
    ...
    async fn create_context(
        ...
        Err(pageknot_model::PageKnotError::new(
...
async fn local_browser_recycles_after_the_configured_job_threshold() -> TestResult {
    ...
    let mut pageknot = PageKnot::builder()
        .browser_backend(backend)
        .browser_recycle_after_jobs(1)
        .build()?;
    let state = Arc::get_mut(&mut pageknot.state)
    ...
    pageknot.state.recycle_idle_browser_if_due().await?;
