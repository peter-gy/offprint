...
use pageknot_browser::BrowserBackend;
use pageknot_chromium::ChromiumDiscovery;
use pageknot_model::{
    BrowserChannel, BrowserInstallationPolicy, BrowserSpec, CaptureId, CaptureProfile,
    EffectiveConfigValue, ErrorStage, NetworkPolicy, PageKnotError, Result,
};
...
pub(crate) fn operation_cancelled_error() -> PageKnotError {
    PageKnotError::new(
        "pageknot.runtime.cancelled",
        ErrorStage::Shutdown,
        "PageKnot operation was cancelled",
    )
}

fn shutdown_deadline_error(owner: &'static str) -> PageKnotError {
    PageKnotError::new(
        "pageknot.runtime.shutdown",
        ErrorStage::Shutdown,
...
mod tests {
    ...
    use pageknot_model::{BrowserChannel, BrowserInstallationPolicy, CaptureId};
    use std::error::Error;
    use std::sync::Arc;
    ...
    async fn close_cancels_runtime_operations() -> TestResult {
        ...
        let cancellation = pageknot.state.operation_cancellation();
    ...
    async fn duplicate_job_cleanup_counts_one_completion() -> TestResult {
        ...
        let control = Arc::new(JobControl::new(pageknot.state.operation_cancellation()));
        pageknot.state.register_job(capture_id.clone(), control)?;

        pageknot.state.unregister_job(&capture_id);
        pageknot.state.unregister_job(&capture_id);
        tokio::task::yield_now().await;

        assert_eq!(pageknot.state.completed_jobs.load(Ordering::Acquire), 1);
        pageknot.close().await?;
        Ok(())
    }
}
