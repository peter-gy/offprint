use offprint_model::{
    ArtifactManifest, BrowserProduct, ContentDigest, ErrorStage, OffprintError, ResourceSummary,
...
pub(super) struct PdfMetadata {
    ...
    pub(super) capture_policy_sha256: ContentDigest,
    ...
    pub(super) verification_mode: VerificationMode,
...
impl PdfMetadata {
    ...
    pub(super) fn identifier(&self) -> String {
        format!(
            "urn:offprint:source-artifact:sha256:{}",
...
pub(super) const fn verification_mode(level: VerificationMode) -> &'static str {
    match level {
...
pub(super) fn parse_verification_mode(value: &str) -> Option<VerificationMode> {
    match value {
        "static" => Some(VerificationMode::Static),
        "offline" => Some(VerificationMode::Offline),
        _ => None,
    }
}
