...
use offprint_model::{ArtifactFormat, ArtifactManifest, ContentDigest, Result};
...
pub fn encode_mhtml(html: &[u8], manifest: &ArtifactManifest) -> Result<Vec<u8>> {
    let html = offprint_html::encode_sandboxed_html(html)?;
    let digest = ContentDigest::sha256(&html);
    let boundary = format!("offprint-{digest}");
    ...
    let message = format!(
        "From: <Saved by Offprint>\r\n\
Subject: Offprint capture\r\n\
MIME-Version: 1.0\r\n\
...
{html_body}\r\n\
--{boundary}\r\n\
Content-Type: application/vnd.offprint.manifest+json\r\n\
Content-Transfer-Encoding: base64\r\n\
Content-Location: offprint-manifest.json\r\n\
\r\n\
{manifest_body}\r\n\
--{boundary}--\r\n"
    );
...
/// Verifies MHTML framing, exact parts, and the contained Offprint manifest.
pub fn verify_mhtml(bytes: &[u8]) -> Result<FormatEvidence> {
    ...
    let Some((header_source, body)) = source.split_once("\r\n\r\n") else {
        return ensure_verified(ArtifactFormat::Mhtml, false, false);
    };
    ...
    if headers.keys().cloned().collect::<BTreeSet<_>>() != expected_header_names
        || headers.get("from").map(String::as_str) != Some("<Saved by Offprint>")
        || headers.get("subject").map(String::as_str) != Some("Offprint capture")
    ...
    let expected_boundary = format!("offprint-{}", ContentDigest::sha256(&html));
    ...
    let structure_valid = boundary == expected_boundary
        ...
        && parts[1].headers.get("content-location").map(String::as_str)
            == Some("offprint-manifest.json");
    let content_valid = offprint_html::verify_static_sandboxed(&html).is_ok()
        && manifest_matches
        && sidecar_manifest
            .as_ref()
    ...
    ensure_verified(ArtifactFormat::Mhtml, structure_valid, content_valid)
}
...
mod tests {
    ...
    fn mhtml_verifier_rejects_manifest_mutation_and_extra_parts() -> Result<()> {
        ...
        let manifest_marker = b"Content-Location: offprint-manifest.json\r\n\r\n";
        ...
        let sandboxed = offprint_html::encode_sandboxed_html(&html)?;
        let boundary = format!(
            "offprint-{}",
            offprint_model::ContentDigest::sha256(&sandboxed)
        );
