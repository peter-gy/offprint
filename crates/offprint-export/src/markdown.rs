...
pub fn encode_markdown(
    ...
    if options.front_matter {
        ...
        markdown.push_str("\"\n");
        markdown.push_str("capture_policy_sha256: \"");
        markdown.push_str(&manifest.capture_policy_sha256.to_hex());
        markdown.push_str("\"\n---\n\n");
    }
    let body = document
...
pub fn verify_markdown(bundle: &MarkdownBundle) -> Result<FormatEvidence> {
    ...
    ensure_verified(ArtifactFormat::Markdown, structure_valid, content_valid)
}
...
mod tests {
    ...
    use offprint_model::{
        ArtifactManifest, ContentDigest, ErrorStage, MarkdownOptions, OffprintError, Result,
    };
    ...
    fn fixture_from_html(source: &[u8]) -> Result<(Vec<u8>, ArtifactManifest)> {
        ...
        let html = offprint_html::encode_html(&document, &manifest)?;
        Ok((html, manifest))
    }
