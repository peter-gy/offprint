...
use offprint_document::{Document, discover_css_resources, discover_document_resources};
use offprint_model::{ContentDigest, Result};
use url::Url;

use super::active_content::validate_unowned_content;
...
pub(super) fn validate_embedded_resource(
    ...
    *embedded_resources.entry(key).or_default() += 1;
    ...
    let nested_urls = if media_type == "text/css" {
        ...
        validate_unowned_content(&document)?;
        ...
        discover_document_resources(&document, base_url)
            .map_err(|error| {
                verification_error(
                    "offprint.verification.reference",
                    "embedded HTML contains an invalid resource reference",
                )
                .with_source(error)
            })?
            .resources()
