...
const FRAME_ID_ATTRIBUTE: &str = "data-pageknot-frame-id";
const FRAME_BASE_ATTRIBUTE: &str = "data-pageknot-frame-base";
const CSS_BASE_ATTRIBUTE: &str = "data-pageknot-css-base";
...
pub(super) fn validate_csp(
    ...
    let policies = document
        ...
        .filter_map(|id| {
            ...
            {
                attribute_value(attrs, "content").map(str::to_owned)
            } else {
                None
            }
        })
    ...
    if policies.as_slice() != [expected] {
        return Err(verification_error(
            "pageknot.verification.csp",
            "artifact content security policy is missing or inconsistent",
        ));
    }
    Ok(())
}
...
pub(super) fn validate_artifact_documents(
    ...
    let base_url = Url::parse("file:///pageknot-artifact.html").map_err(|error| {
        verification_error(
...
fn validate_structural_repair(document: &Document, manifest: &ArtifactManifest) -> Result<bool> {
    ...
    validate_repair_tree(&repair).map_err(|error| {
        verification_error("pageknot.verification.structural_repair", error.message)
    })?;
