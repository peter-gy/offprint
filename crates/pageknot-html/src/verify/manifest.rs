...
use pageknot_model::{
    ArtifactKind, ArtifactManifest, ContentDigest, ResourceOutcome, ResourceSummary, Result,
    ViewState,
};
...
fn validate_manifest(manifest: &ArtifactManifest) -> Result<()> {
    if manifest.schema_version != pageknot_model::PUBLIC_SCHEMA_VERSION
        || manifest.format.version != pageknot_model::ARTIFACT_FORMAT_VERSION
        || manifest.format.kind != ArtifactKind::Html
    {
        return Err(verification_error(
            "pageknot.verification.format",
    ...
    if !manifest.verification.passed || manifest.verification.network_requests != 0 {
        return Err(verification_error(
            "pageknot.verification.record",
            "artifact manifest does not record a successful zero-network verification",
        ));
    }
    Ok(())
}
        ...
        match &record.outcome {
            ResourceOutcome::Embedded {
                ...
                if record.provenance.is_none() {
                    return Err(verification_error(
                        "pageknot.verification.resource_provenance",
