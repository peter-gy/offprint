...
use pageknot_model::{
    ContentDigest, ErrorStage, MAXIMUM_CAPTURE_NODES, PageKnotError, RepairNode, Result,
    StructuralRepairTree,
};

pub const CSP_ELEMENT_ID: &str = "pageknot-csp";
pub const REPAIR_DATA_ELEMENT_ID: &str = "pageknot-repair-data";
pub const REPAIR_SCRIPT_ELEMENT_ID: &str = "pageknot-repair-script";
pub const REPAIR_MEDIA_TYPE: &str = "application/vnd.pageknot.repair+json";
pub const REPAIR_MARKER_ATTRIBUTE: &str = "data-pageknot-node";

pub const RESTORATION_SCRIPT: &str = r#"(() => {
  "use strict";
  const data = document.getElementById("pageknot-repair-data");
  if (!data) return;
  ...
  const index = (root) => {
    for (const node of root.childNodes) {
      ...
      const marker = node.getAttribute("data-pageknot-node");
  ...
  const clearMarkers = (root) => {
    for (const node of root.childNodes) {
      if (node.nodeType !== Node.ELEMENT_NODE) continue;
      node.removeAttribute("data-pageknot-node");
  ...
  const restore = () => {
    ...
    document.documentElement.removeAttribute("data-pageknot-node");
...
fn repair_error(message: impl Into<String>) -> PageKnotError {
    PageKnotError::new(
        "pageknot.transform.structural_repair",
...
mod tests {
    ...
    fn valid_repair_data_adds_the_owned_script() {
        let source = format!(
            ...
            }}</script></head><body data-pageknot-node="2"></body></html>"#
        );
