...
use pageknot_model::{ArtifactManifest, ContentDigest, VerificationResult};
...
impl<B> VerifiedHtmlProof<B> {
    ...
    #[must_use]
    pub const fn verification(&self) -> &VerificationResult {
        &self.verification
    }

    /// Decomposes the proof into its bound values.
    #[must_use]
    pub fn into_parts(self) -> (B, ArtifactManifest, ContentDigest, VerificationResult) {
...
impl<B> VerifiedHtml<B> {
    ...
    pub(crate) fn into_verification_and_manifest(self) -> (VerificationResult, ArtifactManifest) {
