...
use pageknot_model::{
    ArtifactResult, ConflictPolicy, ContentDigest, ERROR_CODE_REGISTRY, ErrorStage, PageKnotError,
    Result,
};
...
impl FileArtifactWriter {
    pub fn create(destination: impl Into<Utf8PathBuf>, conflict: ConflictPolicy) -> Result<Self> {
        ...
        let staging = Builder::new()
            .prefix(&format!(".{file_name}.pageknot-"))
            .tempfile_in(parent)
            .map_err(|error| {
...
fn registered_output_io_error(
    ...
    {
        Some(definition) => PageKnotError::new(definition.code, definition.stage, message)
            .retryable(definition.retryable),
        None => PageKnotError::new(code, ErrorStage::Internal, message),
    };
...
mod tests {
    ...
    fn staging_creation_failure_uses_canonical_registry_metadata() -> std::io::Result<()> {
        ...
        let definition = ERROR_CODE_REGISTRY
            .iter()
            .find(|definition| definition.code == "pageknot.output.staging")
    ...
    fn commit_rejects_same_length_staging_tampering() -> std::io::Result<()> {
        ...
        assert_eq!(error.code.as_str(), "pageknot.artifact.digest");
    ...
    fn commit_rejects_staging_path_replacement() -> std::io::Result<()> {
        ...
        assert_eq!(error.code.as_str(), "pageknot.output.staging");
        assert!(!destination.exists());
        Ok(())
    }
    ...
    fn uniquify_commits_to_the_first_available_portable_name() -> std::io::Result<()> {
        ...
        let path = match result.as_ref().ok() {
            Some(pageknot_model::ArtifactResult::File { path, .. }) => Some(path),
            _ => None,
        };
        assert_eq!(
    ...
    fn output_parent_symlink_is_rejected() -> std::io::Result<()> {
        ...
        assert_eq!(error.code.as_str(), "pageknot.output.directory");
        Ok(())
    }
    ...
    fn replace_rejects_a_symbolic_link_destination() -> std::io::Result<()> {
        ...
        assert_eq!(error.code.as_str(), "pageknot.output.path");
