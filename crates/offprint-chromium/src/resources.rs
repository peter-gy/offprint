...
use offprint_browser::{LoadedResource, ResourceObservationLimits};
use offprint_model::{
    ErrorStage, OffprintError, RedactionPolicy, ResourceRetrievalSource, Result, SourceSummary,
};
...
async fn handle_body_task_completion(
    completed: Option<std::result::Result<offprint_model::Result<()>, tokio::task::JoinError>>,
    state: &Mutex<ObservedResourceState>,
    changed: &Notify,
) -> bool {
    let error = match completed {
        ...
        Some(Err(error)) => Some(OffprintError::new(
            "offprint.browser.resource_task",
            ErrorStage::Resource,
    ...
    pub(crate) async fn capture_intercepted_response(
        ...
        while let Some(chunk) = tokio_stream::StreamExt::next(&mut stream).await {
            let chunk = match chunk {
                Ok(chunk) => chunk,
                Err(error) => {
                    if error.code.as_str() == "offprint.resource.limit"
...
pub(crate) fn with_resource_url(mut error: OffprintError, url: &Url) -> OffprintError {
...
fn with_intercepted_resource_url(error: OffprintError, parameters: &Value) -> OffprintError {
    let url = parameters
        .pointer("/request/url")
        .and_then(Value::as_str)
...
fn redact_url_from_error(error: &mut OffprintError, raw: &str, redacted: &str) {
    ...
    -> std::result::Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(
            ...
            Some(&serde_json::json!(
                offprint_model::ContentDigest::sha256(url.as_str()).to_string()
            ))
        );
        Ok(())
    }
