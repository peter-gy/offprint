...
use offprint_browser::{
    AttachedFrame, LoadedResource, NavigationResult, NetworkGuard, ObservationLimits,
...
impl ChromiumPage {
    ...
    pub async fn enable_network_guard(
        ...
        if !self.client.is_owned_browser() && !matches!(guard.policy(), NetworkPolicy::Unrestricted)
        {
            return Err(OffprintError::new(
                ...
                ErrorStage::Validation,
                "restricted network policy requires a browser process owned by Offprint",
            ));
        }
        let mut interception = self.interception.lock().await;
        if interception.is_some() {
            return Err(OffprintError::new(
                "offprint.navigation.policy_state",
    ...
    pub(crate) async fn collector_call_in_session(
        ...
        let expression = format!("__offprintCollector.call({method}, {})", arguments);
    ...
    async fn ensure_session_runtime(&self, session_id: &str) -> Result<()> {
        ...
        loop {
            ...
            match result {
                Ok(_) => return Ok(()),
                Err(error)
                    if error.code.as_str() == "offprint.browser.cdp_timeout"
                ...
                Err(error) => {
                    let busy = error.code.as_str() == "offprint.browser.cdp_timeout";
                    return Err(OffprintError::new(
                        "offprint.collector.evaluate",
                        ErrorStage::Collection,
                        if busy {
    ...
    pub async fn capture_visual_fallback(
        ...
        if clip.width * clip.height > MAXIMUM_SCREENSHOT_PIXELS {
            return Err(OffprintError::new(
                "offprint.visual_fallback.pixel_limit",
                ErrorStage::Collection,
        ...
        let encoded = response
            ...
            .ok_or_else(|| {
                OffprintError::new(
                    "offprint.visual_fallback.protocol_shape",
                    ErrorStage::Collection,
        ...
        let bytes = base64::engine::general_purpose::STANDARD
            ...
            .map_err(|error| {
                OffprintError::new(
                    "offprint.visual_fallback.encoding",
                    ErrorStage::Collection,
    ...
    pub async fn capture_page_screenshot(&self, maximum_bytes: u64) -> Result<Vec<u8>> {
        ...
        let encoded = response
            ...
            .ok_or_else(|| {
                OffprintError::new(
                    "offprint.screenshot.protocol_shape",
...
impl PageSession for ChromiumPage {
    ...
    async fn observe_frame(
        ...
        content: &offprint_model::ContentPolicy,
    ) -> Result<ObservedFrame> {
        crate::collector::collect_frame_observation_with_metadata(
            self, session_id, frame_id, capture_id, limits, content,
        )
        .await
    }
    ...
    async fn load_resource_in_session(
        &self,
        session_id: &str,
        browser_frame_id: &str,
        url: &Url,
        maximum_bytes: u64,
    ) -> Result<LoadedResource> {
        ChromiumPage::load_resource_in_session(
            self,
            session_id,
            browser_frame_id,
            url,
