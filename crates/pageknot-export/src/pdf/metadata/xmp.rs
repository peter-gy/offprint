...
use pageknot_model::{
    BrowserProduct, ContentDigest, ErrorStage, PageKnotError, ResourceSummary, Result,
    VerificationPolicy,
};
...
use super::model::{
    ...
    parse_verification_level, verification_level,
};
...
const PAGEKNOT_NAMESPACE: &[u8] = b"https://github.com/peter-gy/pageknot/xmp/pdf/1.0/";
    ...
    let write = (|| -> io::Result<()> {
        ...
        write_start(
            ...
            &[
                ...
                (
                    "xmlns:pageknot",
                    "https://github.com/peter-gy/pageknot/xmp/pdf/1.0/",
                ),
            ],
        )?;
        ...
        write_text_element(&mut writer, "pageknot:SourceFinalURL", &metadata.final_url)?;
        write_text_element(
            &mut writer,
        ...
        write_text_element(&mut writer, "pageknot:CaptureLocale", &metadata.locale)?;
        write_text_element(&mut writer, "pageknot:CaptureTimezone", &metadata.timezone)?;
        write_text_element(
            &mut writer,
...
pub(super) fn decode_xmp(bytes: &[u8]) -> Result<PdfMetadata> {
    ...
    let requested_url = required_text(description, PAGEKNOT_NAMESPACE, b"SourceRequestedURL")?;
    let final_url = required_text(description, PAGEKNOT_NAMESPACE, b"SourceFinalURL")?;
    ...
    let policy_sha256 = required_digest(description, b"PolicySHA256")?;
    let captured_at = required_text(description, PAGEKNOT_NAMESPACE, b"CaptureTimestamp")?;
    let generator_name = required_text(description, PAGEKNOT_NAMESPACE, b"GeneratorName")?;
    let generator_version = required_text(description, PAGEKNOT_NAMESPACE, b"GeneratorVersion")?;
    ...
    let browser_version = required_text(description, PAGEKNOT_NAMESPACE, b"BrowserVersion")?;
    let browser_revision = optional_text(description, PAGEKNOT_NAMESPACE, b"BrowserRevision")?;
    let protocol_version = required_text(description, PAGEKNOT_NAMESPACE, b"ProtocolVersion")?;
    let locale = required_text(description, PAGEKNOT_NAMESPACE, b"CaptureLocale")?;
    let timezone = required_text(description, PAGEKNOT_NAMESPACE, b"CaptureTimezone")?;
    let verification_level = required_verification_level(description)?;
    ...
    let warning_codes = optional_array(description, PAGEKNOT_NAMESPACE, b"WarningCodes", b"Bag")?
        .unwrap_or_default();
    let source = SourceMetadata {
        ...
        language,
        rights,
        site_name: optional_text(description, PAGEKNOT_NAMESPACE, b"SourceSiteName")?,
        source_generator: optional_text(description, PAGEKNOT_NAMESPACE, b"SourceGenerator")?,
        published_at: optional_text(description, PAGEKNOT_NAMESPACE, b"SourcePublishedDate")?,
        modified_at: optional_text(description, PAGEKNOT_NAMESPACE, b"SourceModifiedDate")?,
    };
...
fn required_digest(node: &XmlNode, local: &[u8]) -> Result<ContentDigest> {
    required_text(node, PAGEKNOT_NAMESPACE, local)?
        .parse()
...
fn required_boolean(node: &XmlNode, local: &[u8]) -> Result<bool> {
    let value = required_text(node, PAGEKNOT_NAMESPACE, local)?;
    parse_boolean(&value)
...
fn required_browser_product(node: &XmlNode) -> Result<BrowserProduct> {
    let value = required_text(node, PAGEKNOT_NAMESPACE, b"BrowserProduct")?;
    parse_browser_product(&value).ok_or_else(|| xmp_error("XMP browser product is invalid"))
}

fn required_verification_level(node: &XmlNode) -> Result<VerificationPolicy> {
    let value = required_text(node, PAGEKNOT_NAMESPACE, b"VerificationLevel")?;
    parse_verification_level(&value).ok_or_else(|| xmp_error("XMP verification level is invalid"))
}
...
fn xmp_error(message: impl Into<String>) -> PageKnotError {
    PageKnotError::new(
