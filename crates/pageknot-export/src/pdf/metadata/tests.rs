...
use pageknot_model::{ArtifactManifest, ContentDigest, ErrorStage};

use super::source::SourceMetadata;
use super::{embed_pdf_metadata, verify_pageknot_pdf};
...
#[test]
fn enriched_pdf_has_cross_checked_pageknot_metadata() -> TestResult {
...
fn verifier_rejects_independent_duplicate_field_mutations() -> TestResult {
    ...
    let mutations = [
        ...
        (
            "semantic page count",
            mutate_xmp(&enriched, |xmp| {
                replace_xmp_text(xmp, "pageknot:PageCount", "2")
            })?,
        ),
    ];
    ...
    for (name, mutated) in mutations {
        assert!(
            verify_pageknot_pdf(&mutated).is_err(),
...
fn verifier_rejects_nested_semantic_values_and_wrong_namespaces() -> TestResult {
    ...
    assert!(verify_pageknot_pdf(&nested).is_err());
    assert!(verify_pageknot_pdf(&wrong_namespace).is_err());
    Ok(())
}
