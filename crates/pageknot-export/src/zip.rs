...
pub fn encode_zip(html: &[u8], manifest: &ArtifactManifest) -> Result<Vec<u8>> {
    let mut output = Cursor::new(Vec::new());
    {
        ...
        archive
            .start_file("pageknot-manifest.json", options)
            .map_err(zip_error)?;
...
/// Verifies exact ZIP members and the contained PageKnot HTML artifact.
pub fn verify_zip(bytes: &[u8]) -> Result<VariantEvidence> {
    let mut archive = zip::ZipArchive::new(Cursor::new(bytes)).map_err(zip_verify_error)?;
    let expected_names = ["index.html", "pageknot-manifest.json"];
    ...
    if !structure_valid {
        return ensure_verified(ArtifactVariantKind::Zip, false, false);
    }
    ...
    let content_valid = pageknot_html::verify_static(&html).is_ok()
    ...
    ensure_verified(ArtifactVariantKind::Zip, structure_valid, content_valid)
}
...
fn zip_error(error: zip::result::ZipError) -> PageKnotError {
    PageKnotError::new(
        "pageknot.export.zip",
        ErrorStage::Encoding,
...
fn zip_verify_error(error: zip::result::ZipError) -> PageKnotError {
    export_error(
        "pageknot.export.verify",
