...
use pageknot_model::{ArtifactVariantKind, ContentDigest, Result};
...
<title>Opening PageKnot capture</title>
<script id="pageknot-compressed" type="application/octet-stream" data-sha256="{digest}">{payload}</script>
<script>
(() => {{
  const encoded = document.getElementById("pageknot-compressed").textContent.trim();
...
/// Verifies and decompresses a self-extracting PageKnot shell.
pub fn verify_self_extracting(bytes: &[u8]) -> Result<VariantEvidence> {
    ...
    let marker =
        r#"<script id="pageknot-compressed" type="application/octet-stream" data-sha256=""#;
    let Some(start) = source.find(marker).map(|index| index + marker.len()) else {
        return ensure_verified(ArtifactVariantKind::SelfExtracting, false, false);
    };
    ...
    let content_valid = u64::try_from(html.len()).unwrap_or(u64::MAX) <= MAXIMUM_DECODED_HTML_BYTES
        ...
        && pageknot_html::verify_static(&html).is_ok();
    ensure_verified(
...
mod tests {
    ...
    fn fixture() -> Result<Vec<u8>> {
        ...
        pageknot_html::encode_html(&document, &manifest)
    }
