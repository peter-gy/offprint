...
use offprint_document::{NodeData, serialize_document};
    ...
    let csp = document
        ...
        .ok_or_else(|| {
            OffprintError::new(
                "offprint.artifact.sandbox",
                ErrorStage::Encoding,
                "sandboxed HTML has no Offprint content security policy",
            )
        })?;
...
mod tests {
    ...
    fn sandboxed_html_keeps_view_state_without_an_executable_restorer() {
        ...
        assert!(
            html.as_ref()
                .is_ok_and(|html| html.contains(r#"data-offprint-scroll-x="8""#))
        );
        assert!(
            html.as_ref()
                .is_ok_and(|html| html.contains(r#"data-offprint-scroll-y="144""#))
        );
        assert!(
            sandboxed
                .as_deref()
