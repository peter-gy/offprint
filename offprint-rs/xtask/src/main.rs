...
async fn run(arguments: Arguments) -> Result<(), String> {
    match arguments.command {
        ...
        Command::CheckRepository => repository::check(&repository_root()),
        Command::Package {
            target,
            binary,
            output,
        } => package::create(&repository_root(), &target, &binary, &output),
        Command::VerifyPackageMetadata {
            metadata,
            target,
            source_revision,
        } => package::verify_metadata(&repository_root(), &metadata, &target, &source_revision),
        Command::VerifyCratePackages { directory } => {
            package::verify_crate_packages(&repository_root(), &directory)
        }
    }
}
...
fn fixture_command(runner: &offprint_test_support::FixtureRunner) -> ProcessCommand {
    ...
    command
        .current_dir(repository_root().join("offprint-rs"))
...
fn example_documents() -> Result<BTreeMap<&'static str, Vec<u8>>, String> {
    ...
    insert_example(
        ...
        json!({
            ...
            "generator": {"name": "Offprint", "version": env!("CARGO_PKG_VERSION")},
            "source": source,
