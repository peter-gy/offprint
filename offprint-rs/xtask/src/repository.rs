...
use crate::package::{CRATE_LICENSE, PUBLISHABLE_CRATES, crate_directory};
...
pub fn check(root: &Path) -> Result<(), String> {
    ...
    check_development_docs_inventory(root, &files, &mut violations)?;
    for path in &files {
...
fn check_development_docs_inventory(
    root: &Path,
    files: &[std::path::PathBuf],
    violations: &mut Vec<String>,
) -> Result<(), String> {
    let documentation_root = "development_docs";
    let index_name = "README.md";
    let index_path = root.join(documentation_root).join(index_name);
    ...
    for path in files {
        ...
        if !relative.starts_with(documentation_root)
            || relative == Path::new(documentation_root).join(index_name)
...
fn check_binding_contract_wiring(root: &Path, violations: &mut Vec<String>) -> Result<(), String> {
    let node_path = root.join("sdk/node/index.d.ts");
    ...
    let python_path = root.join("sdk/python/src/offprint/__init__.pyi");
...
fn check_crate_package_metadata(root: &Path, violations: &mut Vec<String>) -> Result<(), String> {
    let workspace_path = root.join("offprint-rs/Cargo.toml");
    ...
    let test_support_path = root.join("offprint-rs/test-support/Cargo.toml");
    ...
    let test_support_member = workspace_table
        ...
        .is_some_and(|members| {
            members
                .iter()
                .any(|member| member.as_str() == Some("test-support"))
        });
    if !test_support_member {
        violations
            .push("offprint-rs/Cargo.toml must keep test-support as a workspace member".to_owned());
    }
    ...
    if test_support_dependency != Some("test-support") {
        violations.push(
            "offprint-rs/Cargo.toml must keep offprint-test-support as a workspace path dependency"
                .to_owned(),
        );
    }
    Ok(())
...
fn check_version_alignment(root: &Path, violations: &mut Vec<String>) -> Result<(), String> {
    ...
    let cargo = fs::read_to_string(root.join("offprint-rs/Cargo.toml"))
    ...
    let node = fs::read_to_string(root.join("sdk/node/package.json"))
    ...
    let python = fs::read_to_string(root.join("sdk/python/pyproject.toml"))
...
"#,
        ...
        let test_support = temporary.path().join("offprint-rs/test-support");
        ...
        fs::write(
            temporary.path().join("offprint-rs/model/LICENSE"),
            b"different license\n",
        )
        .map_err(|error| error.to_string())?;
        check_crate_package_metadata(temporary.path(), &mut violations)?;
        ...
        fs::write(
            temporary
                .path()
                .join("offprint-rs/model/Cargo.toml"),
