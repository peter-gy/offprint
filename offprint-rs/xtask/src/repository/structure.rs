...
fn check_dependency_direction(root: &Path, violations: &mut Vec<String>) -> Result<(), String> {
    ...
    let output = Command::new(cargo)
        .current_dir(root.join("offprint-rs"))
...
fn is_generated(path: &Path) -> bool {
    path.starts_with("offprint-rs/chromium/src/cdp/generated")
        || path.starts_with("collector/dist")
        || path.starts_with("offprint-rs/chromium/generated")
        || path == Path::new("sdk/node/contracts.generated.d.ts")
        || path == Path::new("sdk/python/src/offprint/contracts.py")
...
mod tests {
    ...
    fn size_exemptions_are_explicit() {
        assert!(is_generated(Path::new(
            "offprint-rs/chromium/src/cdp/generated/page.rs"
        )));
        assert!(is_generated(Path::new(
            "sdk/python/src/offprint/contracts.py"
        )));
        assert!(is_test(Path::new(
            "offprint-rs/core/tests/fixture_matrix.rs"
        )));
        assert!(is_test(Path::new("offprint-rs/cli/src/runner/tests.rs")));
        assert!(!is_generated(Path::new("offprint-rs/core/src/lib.rs")));
        assert!(!is_test(Path::new("offprint-rs/core/src/lib.rs")));
        assert!(is_handwritten_production_source(Path::new(
            "offprint-rs/core/src/lib.rs"
        )));
        assert!(!is_handwritten_production_source(Path::new(
