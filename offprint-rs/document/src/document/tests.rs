...
fn visual_fallback_resolution_reaches_nested_inline_documents() {
    let mut document = Document::parse(
        br#"<iframe srcdoc="<html><body><img data-offprint-visual-fallback='inline-0'></body></html>"></iframe>"#,
    );

    let resolved =
        document.resolve_visual_fallback("inline-0", "data:image/png;base64,b2ZmcHJpbnQ=");
    let html = serialize_document(&document)
        .ok()
        .map(|bytes| String::from_utf8_lossy(&bytes).into_owned());
    ...
    assert!(
        html.as_ref()
            .is_some_and(|html| html.contains("data:image/png;base64,b2ZmcHJpbnQ="))
    );
    assert!(
        html.as_ref()
            .is_some_and(|html| !html.contains("data-offprint-visual-fallback"))
    );
}
