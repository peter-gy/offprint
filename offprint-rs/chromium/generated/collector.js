(function() {
	...
	const repairDataElementId = "offprint-repair-data";
	const repairMediaType = "application/vnd.offprint.repair+json";
	const repairMarkerAttribute = "data-offprint-node";
	const documentScrollXAttribute = "data-offprint-scroll-x";
	const documentScrollYAttribute = "data-offprint-scroll-y";
	const elementScrollLeftAttribute = "data-offprint-scroll-left";
	const elementScrollTopAttribute = "data-offprint-scroll-top";
	const animationMarkerAttribute = "data-offprint-animation";
	const animationStyleAttribute = "data-offprint-animation-style";
	const capturedShadowModeAttribute = "data-offprint-shadow-mode";
	...
	function captureOptionalGetter(prototype, name, unavailable) {
		const getter = prototype ? propertyDescriptor(prototype, name)?.get : void 0;
	...
	function captureMethod(prototype, name, fallback) {
		if (!prototype) return fallback;
		const method = prototype ? propertyDescriptor(prototype, name)?.value : void 0;
		if (!method) return () => {
	...
	const setForEach = uncurryThis(Set.prototype.forEach);
	const setSize = captureGetter(Set.prototype, "size", (set) => set.size);
	...
	const stringToLocaleLowerCase = uncurryThis(String.prototype.toLocaleLowerCase);
	const regExpTest = uncurryThis(RegExp.prototype.test);
	const typedArraySubarray = uncurryThis(Uint8Array.prototype.subarray);
	const safeReflectApply = reflectApply;
	const typedArrayByteLength = captureGetter(Uint8Array.prototype, "byteLength", (bytes) => bytes.byteLength);
	...
	function stringReplacePattern(value, search, replacement) {
		return reflectApply(stringReplaceMethod, value, [search, replacement]);
	}
	function stringMatch(value, pattern) {
		return reflectApply(stringMatchMethod, value, [pattern]);
	}
	...
	function encodeUtf8Chunks(chunks, byteLength) {
		...
		for (let index = 0; index < chunks.length; index += 1) {
			...
			if (encoded.read !== chunks[index].length) throw new SafeTypeError("bounded UTF-8 encoding did not consume its input");
	...
	const documentTypePrototype = typeof DocumentType === "undefined" ? void 0 : DocumentType.prototype;
	const nodeListPrototype = typeof NodeList === "undefined" ? void 0 : NodeList.prototype;
	...
	const firstChild = captureGetter(nodePrototype, "firstChild", (node) => node.firstChild);
	const nextSibling = captureGetter(nodePrototype, "nextSibling", (node) => node.nextSibling);
	const lastChild = captureGetter(nodePrototype, "lastChild", (node) => node.lastChild);
	const previousSibling = captureGetter(nodePrototype, "previousSibling", (node) => node.previousSibling);
	const parentNode = captureGetter(nodePrototype, "parentNode", (node) => node.parentNode);
	const ownerDocument = captureGetter(nodePrototype, "ownerDocument", (node) => node.ownerDocument);
	const nodeValue = captureGetter(nodePrototype, "nodeValue", (node) => node.nodeValue);
	captureGetter(nodePrototype, "textContent", (node) => node.textContent);
	const setNodeTextContent = captureSetter(nodePrototype, "textContent", (node, value) => {
		node.textContent = value;
	});
	const getNodeChildNodes = captureGetter(nodePrototype, "childNodes", (node) => node.childNodes);
	const getNodeBaseUri = captureGetter(nodePrototype, "baseURI", (node) => node.baseURI);
	const getNodeNamespace = captureGetter(elementPrototype$1, "namespaceURI", (node) => node.namespaceURI);
	const getNodeLocalName = captureGetter(elementPrototype$1, "localName", (node) => node.localName);
	const cloneNode = captureMethod(nodePrototype, "cloneNode", (node, deep) => node.cloneNode(deep));
	const appendChild = captureMethod(nodePrototype, "appendChild", (node, child) => node.appendChild(child));
	...
	function querySelector(root, selector) {
		...
		if (type === 9) return documentQuerySelector(root, selector);
		if (type === 11) return fragmentQuerySelector(root, selector);
	...
	function querySelectorAll(root, selector) {
		...
		for (let index = 0; index < length; index += 1) {
			...
			if (node) arrayPush(result, node);
		}
		return result;
	}
	const setAttribute = captureMethod(elementPrototype$1, "setAttribute", (element, name, value) => {
		element.setAttribute(name, value);
	});
	const getAttribute = captureMethod(elementPrototype$1, "getAttribute", (element, name) => element.getAttribute(name));
	const hasAttribute = captureMethod(elementPrototype$1, "hasAttribute", (element, name) => element.hasAttribute(name));
	const removeAttribute = captureMethod(elementPrototype$1, "removeAttribute", (element, name) => {
		element.removeAttribute(name);
	});
	const toggleAttribute = captureMethod(elementPrototype$1, "toggleAttribute", (element, name, force) => element.toggleAttribute(name, force));
	const contains = captureMethod(nodePrototype, "contains", (node, child) => node.contains(child));
	const elementBounds = captureMethod(elementPrototype$1, "getBoundingClientRect", (element) => element.getBoundingClientRect());
	const getElementAttributes = captureGetter(elementPrototype$1, "attributes", (element) => element.attributes);
	const getElementChildren = captureGetter(elementPrototype$1, "children", (element) => element.children);
	const getElementShadowRoot = captureGetter(elementPrototype$1, "shadowRoot", (element) => element.shadowRoot);
	...
	function childNodes(node) {
		...
		for (let index = 0; index < length; index += 1) {
			...
			if (child) arrayPush(result, child);
		}
		return result;
	}
	...
	const nodeListLength = captureGetter(nodeListPrototype, "length", (nodes) => nodes.length);
	const nodeListItem = captureMethod(nodeListPrototype, "item", (nodes, index) => nodes.item(index));
	const htmlCollectionLength = captureGetter(htmlCollectionPrototype, "length", (collection) => collection.length);
	const htmlCollectionItem = captureMethod(htmlCollectionPrototype, "item", (collection, index) => collection.item(index));
	const namedNodeMapLength = captureGetter(namedNodeMapPrototype, "length", (attributes) => attributes.length);
	const namedNodeMapItem = captureMethod(namedNodeMapPrototype, "item", (attributes, index) => attributes.item(index));
	const attrName = captureGetter(attrPrototype, "name", (attribute) => attribute.name);
	const attrValue = captureGetter(attrPrototype, "value", (attribute) => attribute.value);
	const getDoctypeName = captureGetter(documentTypePrototype, "name", (doctype) => doctype.name);
	const getDoctypePublicId = captureGetter(documentTypePrototype, "publicId", (doctype) => doctype.publicId);
	const getDoctypeSystemId = captureGetter(documentTypePrototype, "systemId", (doctype) => doctype.systemId);
	...
	const parseFromString = captureMethod(parserPrototype, "parseFromString", (parser, source, type) => parser.parseFromString(source, type));
	function parseHtml(source) {
		if (!SafeDOMParser) throw new SafeTypeError("DOMParser is unavailable");
		return parseFromString(new SafeDOMParser(), source, "text/html");
	}
	function removeNode(node) {
		const parent = parentNode(node);
		if (parent) removeChild(parent, node);
	}
	function replaceNode(node, replacement) {
		const parent = parentNode(node);
		if (parent) replaceChild(parent, replacement, node);
	}
	...
	const closedRoots = new SafeWeakMap();
	...
	Element.prototype.attachShadow = function(init) {
		...
		const root = safeReflectApply(originalAttachShadow, this, [normalized]);
		if (normalized.mode === "closed") weakMapSet(closedRoots, this, root);
		return root;
	};
	...
	function createInspectableShadowTemplate(root) {
		...
		if (!document) throw new SafeTypeError("a shadow root has no owner document");
		const template = createElement(document, "template");
		setAttribute(template, "shadowrootmode", "open");
		if (shadowMode(root) === "closed") setAttribute(template, capturedShadowModeAttribute, "closed");
		return template;
	}
	//#endregion
	//#region src/web.ts
	const styleSheetListPrototype = typeof StyleSheetList === "undefined" ? void 0 : StyleSheetList.prototype;
	const cssRuleListPrototype = typeof CSSRuleList === "undefined" ? void 0 : CSSRuleList.prototype;
	const cssStyleSheetPrototype = typeof CSSStyleSheet === "undefined" ? void 0 : CSSStyleSheet.prototype;
	const styleSheetPrototype = typeof StyleSheet === "undefined" ? void 0 : StyleSheet.prototype;
	const mediaListPrototype = typeof MediaList === "undefined" ? void 0 : MediaList.prototype;
	const cssRulePrototype = typeof CSSRule === "undefined" ? void 0 : CSSRule.prototype;
	const cssStyleRulePrototype = typeof CSSStyleRule === "undefined" ? void 0 : CSSStyleRule.prototype;
	const cssFontFaceRulePrototype = typeof CSSFontFaceRule === "undefined" ? void 0 : CSSFontFaceRule.prototype;
	const cssStyleDeclarationPrototype = typeof CSSStyleDeclaration === "undefined" ? void 0 : CSSStyleDeclaration.prototype;
	const windowPrototype = typeof Window === "undefined" ? void 0 : Window.prototype;
	const windowPropertySource = (typeof window === "undefined" ? void 0 : window) ?? windowPrototype;
	...
	const linkElementPrototype = typeof HTMLLinkElement === "undefined" ? void 0 : HTMLLinkElement.prototype;
	const inputPrototype = typeof HTMLInputElement === "undefined" ? void 0 : HTMLInputElement.prototype;
	const textAreaPrototype = typeof HTMLTextAreaElement === "undefined" ? void 0 : HTMLTextAreaElement.prototype;
	const optionPrototype = typeof HTMLOptionElement === "undefined" ? void 0 : HTMLOptionElement.prototype;
	const detailsPrototype = typeof HTMLDetailsElement === "undefined" ? void 0 : HTMLDetailsElement.prototype;
	const imagePrototype = typeof HTMLImageElement === "undefined" ? void 0 : HTMLImageElement.prototype;
	const canvasPrototype = typeof HTMLCanvasElement === "undefined" ? void 0 : HTMLCanvasElement.prototype;
	const mediaPrototype = typeof HTMLMediaElement === "undefined" ? void 0 : HTMLMediaElement.prototype;
	const videoPrototype = typeof HTMLVideoElement === "undefined" ? void 0 : HTMLVideoElement.prototype;
	const selectionPrototype = typeof Selection === "undefined" ? void 0 : Selection.prototype;
	const rangePrototype = typeof Range === "undefined" ? void 0 : Range.prototype;
	const treeWalkerPrototype = typeof TreeWalker === "undefined" ? void 0 : TreeWalker.prototype;
	const context2dPrototype = typeof CanvasRenderingContext2D === "undefined" ? void 0 : CanvasRenderingContext2D.prototype;
	const domRectPrototype = typeof DOMRectReadOnly === "undefined" ? void 0 : DOMRectReadOnly.prototype;
	const locationPrototype = typeof Location === "undefined" ? void 0 : Location.prototype;
	const locationPropertySource = (typeof location === "undefined" ? void 0 : location) ?? locationPrototype;
	const styleSheetListLength = captureGetter(styleSheetListPrototype, "length", (list) => list.length);
	const styleSheetListItem = captureMethod(styleSheetListPrototype, "item", (list, index) => list.item(index));
	const cssRuleListLength = captureGetter(cssRuleListPrototype, "length", (list) => list.length);
	const cssRuleListItem = captureMethod(cssRuleListPrototype, "item", (list, index) => list.item(index));
	const getCssRules = captureGetter(cssStyleSheetPrototype, "cssRules", (sheet) => sheet.cssRules);
	const getStyleSheetOwner = captureGetter(styleSheetPrototype, "ownerNode", (sheet) => sheet.ownerNode);
	const getStyleSheetHref = captureGetter(styleSheetPrototype, "href", (sheet) => sheet.href);
	const styleSheetDisabled = captureGetter(styleSheetPrototype, "disabled", (sheet) => sheet.disabled);
	const getStyleSheetMedia = captureGetter(styleSheetPrototype, "media", (sheet) => sheet.media);
	const getMediaText = captureGetter(mediaListPrototype, "mediaText", (media) => media.mediaText);
	function styleSheetMedia(sheet) {
		return getMediaText(getStyleSheetMedia(sheet));
	}
	const getRuleType = captureGetter(cssRulePrototype, "type", (rule) => rule.type);
	const getRuleText = captureGetter(cssRulePrototype, "cssText", (rule) => rule.cssText);
	const getSelectorText = captureGetter(cssStyleRulePrototype, "selectorText", (rule) => rule.selectorText);
	const getStyleRuleDeclaration = captureGetter(cssStyleRulePrototype, "style", (rule) => rule.style);
	const getFontFaceDeclaration = captureGetter(cssFontFaceRulePrototype, "style", (rule) => rule.style);
	const getStyleSheetFromStyle = captureGetter(styleElementPrototype, "sheet", (element) => element.sheet);
	const getStyleSheetFromSvgStyle = captureGetter(svgStyleElementPrototype, "sheet", (element) => element.sheet);
	const getStyleSheetFromLink = captureGetter(linkElementPrototype, "sheet", (element) => element.sheet);
	function styleSheetsFromList(list) {
		const sheets = [];
		const length = styleSheetListLength(list);
		for (let index = 0; index < length; index += 1) {
			const sheet = styleSheetListItem(list, index);
			if (sheet) arrayPush(sheets, sheet);
		}
		return sheets;
	}
	...
	function documentFontFaces(source) {
		...
		for (let sheetIndex = 0; sheetIndex < sheets.length; sheetIndex += 1) {
			...
			if (!styleSheetHref(sheet)) continue;
			try {
				...
				for (let ruleIndex = 0; ruleIndex < count; ruleIndex += 1) {
					...
					if (rule && cssRuleType(rule) === 5) setAdd(faces, cssRuleText(rule));
				}
			} catch {}
		}
		return faces;
	...
	function styleSheetFor(element) {
		...
		if (name === "style") {
			if (namespace === "http://www.w3.org/1999/xhtml") return getStyleSheetFromStyle(element);
			if (namespace === "http://www.w3.org/2000/svg") return getStyleSheetFromSvgStyle(element);
			return null;
		}
	...
	const stylePropertyValue = captureMethod(cssStyleDeclarationPrototype, "getPropertyValue", (style, name) => style.getPropertyValue(name));
	const setStyleProperty = captureMethod(cssStyleDeclarationPrototype, "setProperty", (style, name, value, priority) => {
		style.setProperty(name, value, priority);
	});
	const styleText = captureGetter(cssStyleDeclarationPrototype, "cssText", (style) => style.cssText);
	const getElementStyle = captureGetter(htmlElementPrototype, "style", (element) => element.style);
	const getSvgElementStyle = captureGetter(typeof SVGElement === "undefined" ? void 0 : SVGElement.prototype, "style", (element) => element.style);
	...
	const pauseAnimation = captureMethod(animationPrototype, "pause", (animation) => {
		animation.pause();
	});
	const animationEffect = captureGetter(animationPrototype, "effect", (animation) => animation.effect);
	const keyframeTarget = captureGetter(keyframeEffectPrototype, "target", (effect) => effect.target);
	const keyframePseudo = captureOptionalGetter(keyframeEffectPrototype, "pseudoElement", null);
	const keyframes = captureMethod(keyframeEffectPrototype, "getKeyframes", (effect) => effect.getKeyframes());
	...
	const selectionRangeCount = captureGetter(selectionPrototype, "rangeCount", (selection) => selection.rangeCount);
	const selectionRange = captureMethod(selectionPrototype, "getRangeAt", (selection, index) => selection.getRangeAt(index));
	const rangeCollapsed = captureGetter(rangePrototype, "collapsed", (range) => range.collapsed);
	const rangeIntersectsNode = captureMethod(rangePrototype, "intersectsNode", (range, node) => range.intersectsNode(node));
	const treeWalkerNext = captureMethod(treeWalkerPrototype, "nextNode", (walker) => walker.nextNode());
	const treeWalkerCurrent = captureGetter(treeWalkerPrototype, "currentNode", (walker) => walker.currentNode);
	const inputType = captureGetter(inputPrototype, "type", (input) => input.type);
	const inputValue = captureGetter(inputPrototype, "value", (input) => input.value);
	const inputChecked = captureGetter(inputPrototype, "checked", (input) => input.checked);
	const textAreaValue = captureGetter(textAreaPrototype, "value", (textArea) => textArea.value);
	const optionSelected = captureGetter(optionPrototype, "selected", (option) => option.selected);
	const detailsOpen = captureGetter(detailsPrototype, "open", (details) => details.open);
	const imageCurrentSource = captureGetter(imagePrototype, "currentSrc", (image) => image.currentSrc);
	const setImageSource = captureSetter(imagePrototype, "src", (image, value) => {
		image.src = value;
	});
	const setImageWidth = captureSetter(imagePrototype, "width", (image, value) => {
		image.width = value;
	});
	const setImageHeight = captureSetter(imagePrototype, "height", (image, value) => {
		image.height = value;
	});
	const setImageAlt = captureSetter(imagePrototype, "alt", (image, value) => {
		image.alt = value;
	});
	const canvasWidth = captureGetter(canvasPrototype, "width", (canvas) => canvas.width);
	const canvasHeight = captureGetter(canvasPrototype, "height", (canvas) => canvas.height);
	const setCanvasWidth = captureSetter(canvasPrototype, "width", (canvas, value) => {
		canvas.width = value;
	});
	const setCanvasHeight = captureSetter(canvasPrototype, "height", (canvas, value) => {
		canvas.height = value;
	});
	const canvasDataUrl = captureMethod(canvasPrototype, "toDataURL", (canvas, type) => canvas.toDataURL(type));
	const canvasContext = captureMethod(canvasPrototype, "getContext", (canvas, type) => canvas.getContext(type));
	const drawCanvasImage = captureMethod(context2dPrototype, "drawImage", (context, image, x, y) => {
		context.drawImage(image, x, y);
	});
	const mediaCurrentTime = captureGetter(mediaPrototype, "currentTime", (media) => media.currentTime);
	const mediaControls = captureGetter(mediaPrototype, "controls", (media) => media.controls);
	const mediaMuted = captureGetter(mediaPrototype, "muted", (media) => media.muted);
	const mediaCurrentSource = captureGetter(mediaPrototype, "currentSrc", (media) => media.currentSrc);
	const mediaReadyState = captureGetter(mediaPrototype, "readyState", (media) => media.readyState);
	const videoWidth = captureGetter(videoPrototype, "videoWidth", (video) => video.videoWidth);
	const videoHeight = captureGetter(videoPrototype, "videoHeight", (video) => video.videoHeight);
	const videoPoster = captureGetter(videoPrototype, "poster", (video) => video.poster);
	const elementScrollLeft = captureGetter(elementPrototype, "scrollLeft", (element) => element.scrollLeft);
	const elementScrollTop = captureGetter(elementPrototype, "scrollTop", (element) => element.scrollTop);
	const elementClientLeft = captureGetter(elementPrototype, "clientLeft", (element) => element.clientLeft);
	const elementClientTop = captureGetter(elementPrototype, "clientTop", (element) => element.clientTop);
	const elementClientWidth = captureGetter(elementPrototype, "clientWidth", (element) => element.clientWidth);
	const elementClientHeight = captureGetter(elementPrototype, "clientHeight", (element) => element.clientHeight);
	const elementOffsetWidth = captureGetter(htmlElementPrototype, "offsetWidth", (element) => element.offsetWidth);
	const elementOffsetHeight = captureGetter(htmlElementPrototype, "offsetHeight", (element) => element.offsetHeight);
	const windowScrollX = captureGetter(windowPropertySource, "scrollX", (view) => view.scrollX);
	const windowScrollY = captureGetter(windowPropertySource, "scrollY", (view) => view.scrollY);
	const windowFrameElement = captureGetter(windowPropertySource, "frameElement", (view) => view.frameElement);
	const windowInnerWidth = captureGetter(windowPropertySource, "innerWidth", (view) => view.innerWidth);
	const windowInnerHeight = captureGetter(windowPropertySource, "innerHeight", (view) => view.innerHeight);
	const windowDeviceScaleFactor = captureGetter(windowPropertySource, "devicePixelRatio", (view) => view.devicePixelRatio);
	const locationHref = captureGetter(locationPropertySource, "href", (location) => location.href);
	const rectLeft = captureGetter(domRectPrototype, "left", (rect) => rect.left);
	const rectTop = captureGetter(domRectPrototype, "top", (rect) => rect.top);
	const rectWidth = captureGetter(domRectPrototype, "width", (rect) => rect.width);
	const rectHeight = captureGetter(domRectPrototype, "height", (rect) => rect.height);
	//#endregion
	//#region src/motion.ts
	const frozenDocuments = new SafeWeakSet();
	const failedDocuments = new SafeWeakSet();
	const frozenMotion = new SafeWeakMap();
	const generatedMotionStyles = new SafeWeakSet();
	function freezeMotion(source) {
		if (weakSetHas(frozenDocuments, source)) return;
		weakSetAdd(frozenDocuments, source);
		try {
			const animations = animationsWithin(source);
			for (let index = 0; index < animations.length; index += 1) try {
	...
	function freezeDocument(source) {
		...
		if (!style) {
			...
			if (!head) throw new SafeTypeError("render freeze requires an HTML head element");
	...
	function reportMotionCaptureFailure(source, context) {
		if (weakSetHas(failedDocuments, source)) arrayPush(context.warnings, {
	...
	function materializeMotionState(live, clone, context, rules) {
		...
		mapForEach(captured, (properties, pseudo) => {
			if (!pseudo) {
				if (!supportsInlineStyle(clone)) return;
	...
	function motionProperties(animations) {
		...
		for (let index = 0; index < animations.length; index += 1) {
			...
			if (!target || !isElement(target)) continue;
			const pseudo = keyframePseudo(effect) ?? "";
			if (pseudo && !regExpTest(/^::[a-z-]+$/i, pseudo)) continue;
			...
			for (let frameIndex = 0; frameIndex < frames.length; frameIndex += 1) {
				...
				for (let keyIndex = 0; keyIndex < keys.length; keyIndex += 1) {
					...
					if (!matchesKeyframeMetadata(key) && name) setAdd(names, name);
				}
			}
			if (setSize(names) > 0) {
	...
	function cssPropertyName(name) {
		if (stringStartsWith(name, "--")) return name;
	...
	function utf8LengthWithinLimit(value, maximumBytes) {
		if (!numberIsSafeInteger(maximumBytes) || maximumBytes < 0) throw new SafeTypeError("maximumBytes must be a non-negative safe integer");
		let bytes = 0;
		for (let index = 0; index < value.length; index += 1) {
			...
			} else if (codeUnit <= 127) width = 1;
			else if (codeUnit <= 2047) width = 2;
			else width = 3;
			if (bytes > maximumBytes - width) return null;
			bytes += width;
		}
		return bytes;
	}
	...
	function sha256Fallback(bytes) {
		...
		for (let block = 0; block < paddedBytes; block += 64) {
			for (let word = 0; word < 16; word += 1) {
				...
				for (let offset = 0; offset < 4; offset += 1) {
					...
					let byte = 0;
					if (position < byteLength) byte = bytes[position];
					else if (position === byteLength) byte = 128;
					else if (position >= paddedBytes - 8) byte = lengthBytes[position - (paddedBytes - 8)];
					value = value << 8 | byte;
				}
				words[word] = value >>> 0;
			}
		...
		for (let index = 0; index < states.length; index += 1) digest += hexadecimal(states[index], 8);
		return digest;
	}
	...
	var BoundedWriter = class {
		...
		write(value) {
			if (this.overflow || value.length === 0) return !this.overflow;
	...
	function serializeHtmlBounded(root, maximumBytes) {
		...
		while (pending.length > 0) {
			...
			if (!task) continue;
			if (task.kind === "close") {
				if (!writer.write(`</${task.name}>`)) break;
				continue;
			}
			...
			if (type === 8) {
				if (!writer.write("<!--") || !writer.write(nodeValue(node) ?? "") || !writer.write("-->")) break;
				continue;
			}
			if (type !== 1) {
				if (!writer.write(nodeValue(node) ?? "")) break;
				continue;
			}
			const element = node;
			const name = localName(element) ?? "";
			if (!writer.write(`<${name}`)) break;
			const count = attributeCount(element);
			for (let index = 0; index < count; index += 1) {
				const attribute = attributeAt(element, index);
				if (!attribute) continue;
				if (!writer.write(` ${attributeName(attribute)}="`) || !writeEscaped(writer, attributeValue(attribute), "attribute") || !writer.write("\"")) break;
			}
			if (!writer.write(">")) break;
			const isHtml = namespaceUri(element) === "http://www.w3.org/1999/xhtml";
			if (isHtml && setHas(voidElements, name)) continue;
			arrayPush(pending, {
				kind: "close",
				name
			});
			...
			for (let child = lastChild(childRoot); child; child = previousSibling(child)) arrayPush(pending, {
				kind: "node",
				node: child,
				rawText
			});
	...
	function writeJson(writer, value) {
		...
		while (pending.length > 0) {
			...
			if (task.kind === "literal") {
				if (!writer.write(task.value)) return;
				continue;
			}
			if (task.kind === "string") {
			...
			if (current === null) {
				if (!writer.write("null")) return;
			} else if (typeof current === "string") arrayPush(pending, {
				kind: "string",
				value: current
			});
			else if (typeof current === "boolean") {
				if (!writer.write(current ? "true" : "false")) return;
			} else if (typeof current === "number") {
				if (!writer.write(numberIsFinite(current) ? `${current}` : "null")) return;
			...
			} else if (!writer.write("null")) return;
		}
	}
	...
	function removeReservedMetadata(root) {
		...
		walkElements(root, (element) => {
			if (arrayIncludes([
				"offprint-manifest",
				"offprint-repair-data",
				"offprint-repair-script",
				"offprint-state-script"
			], getAttribute(element, "id") ?? "") || hasAttribute(element, "data-offprint-animation-style") && !isGeneratedMotionStyle(element)) arrayPush(remove, element);
		});
		for (let index = 0; index < remove.length; index += 1) removeNode(remove[index]);
	}
	...
	function repairNodesEqual(left, right) {
		...
		while (pending.length > 0) {
			...
			if (leftType !== nodeType(rightNode)) return false;
			if (leftType === 3 || leftType === 8) {
				if ((nodeValue(leftNode) ?? "") !== (nodeValue(rightNode) ?? "")) return false;
				continue;
			}
			if (!isElement(leftNode) || !isElement(rightNode)) return false;
			if (getAttribute(leftNode, "data-offprint-node") !== getAttribute(rightNode, "data-offprint-node") || (namespaceUri(leftNode) ?? htmlNamespace$1) !== (namespaceUri(rightNode) ?? htmlNamespace$1) || (localName(leftNode) ?? "") !== (localName(rightNode) ?? "") || shadowModeFor(leftNode) !== shadowModeFor(rightNode)) return false;
			...
			for (let index = 0; index < leftChildren.length; index += 1) arrayPush(pending, [leftChildren[index], rightChildren[index]]);
			const leftTemplate = templateChildren(leftNode);
			const rightTemplate = templateChildren(rightNode);
			if (leftTemplate.length !== rightTemplate.length) return false;
	...
	function shadowModeFor(node) {
		return isHtmlTemplate(node) ? getAttribute(node, "shadowrootmode") ?? void 0 : void 0;
	}
	function isHtmlTemplate(node) {
		return namespaceUri(node) === htmlNamespace$1 && localName(node) === "template";
	}
	...
	function walkElements(root, visit) {
		...
		while (pending.length > 0) {
			...
			for (let index = 0; index < template.length; index += 1) {
				...
				if (isElement(child)) arrayPush(children, child);
			}
	...
	function appendRepairData(source, root, repairData) {
		...
		if (!head) throw new SafeTypeError("structural repair requires an HTML head element");
	...
	function resolveSelector(source, options) {
		if (options.selector === void 0) return {};
		let target;
		try {
	...
	function retainCloneBranch(target, boundary) {
		...
		while (retained !== boundary) {
			...
			if (!parent) throw new SafeTypeError("selector target clone is detached");
			const siblings = childNodes(parent);
			for (let index = 0; index < siblings.length; index += 1) if (siblings[index] !== retained) removeNode(siblings[index]);
			retained = parent;
		}
	}
	function removeCloneChildren(node) {
		const children = childNodes(node);
		for (let index = 0; index < children.length; index += 1) removeNode(children[index]);
	}
	function applySelectorScope(source, target, cloneRoot, clones) {
		const targetClone = mapGet(clones, target);
		if (!targetClone || !contains(cloneRoot, targetClone)) throw new SafeTypeError("selector target clone is unavailable");
		if (target === documentElement(source)) return;
		const body = documentBody(source);
		if (body && contains(body, target)) {
			const bodyClone = mapGet(clones, body);
			if (!bodyClone) throw new SafeTypeError("document body clone is unavailable");
		...
		if (head && contains(head, target)) {
			...
			if (!headClone) throw new SafeTypeError("document head clone is unavailable");
			retainCloneBranch(targetClone, headClone);
			if (body) {
				const bodyClone = mapGet(clones, body);
				if (bodyClone) removeCloneChildren(bodyClone);
			}
			return;
		}
	...
	function copyState(live, clone, context, snapshotInline) {
		if (isHtmlElement(live, "input") && isHtmlElement(clone, "input")) {
			...
			} else setAttribute(cloneInput, "value", inputValue(liveInput));
			toggleAttribute(cloneInput, "checked", inputChecked(liveInput));
		} else if (isHtmlElement(live, "textarea") && isHtmlElement(clone, "textarea")) setNodeTextContent(clone, textAreaValue(live));
		else if (isHtmlElement(live, "option") && isHtmlElement(clone, "option")) toggleAttribute(clone, "selected", optionSelected(live));
		else if (isHtmlElement(live, "details") && isHtmlElement(clone, "details")) toggleAttribute(clone, "open", detailsOpen(live));
		else if (isHtmlElement(live, "img") && isHtmlElement(clone, "img")) {
			const currentSource = imageCurrentSource(live);
			if (currentSource) {
			...
			try {
				if (!canvasContext(liveCanvas, "2d")) throw new SafeError("canvas pixels require a browser screenshot");
				...
				if (allocatedPayloadBytes > 0) context.budget.settlePayloadAllocation(context.reservation, allocatedPayloadBytes, 0);
				if (context.budget.limitResponse(error)) throw error;
			...
			if (currentSource) setAttribute(cloneMedia, "src", currentSource);
			if (isHtmlElement(live, "video") && isHtmlElement(clone, "video")) materializeVideo(live, clone, context);
		} else if (isHtmlElement(live, "iframe") && isHtmlElement(clone, "iframe")) copyInlineFrame(live, clone, context, snapshotInline);
	}
	function prepareElementState(live, clone) {
		...
		for (let index = 0; index < reserved.length; index += 1) removeAttribute(clone, reserved[index]);
		const document = ownerDocument(live);
		if (document && documentScrollingElement(document) === live) return;
		const scrollLeft = elementScrollLeft(live);
		if (numberIsFinite(scrollLeft) && scrollLeft !== 0) setAttribute(clone, elementScrollLeftAttribute, SafeString(scrollLeft));
		const scrollTop = elementScrollTop(live);
		if (numberIsFinite(scrollTop) && scrollTop !== 0) setAttribute(clone, elementScrollTopAttribute, SafeString(scrollTop));
	}
	function copyInlineFrame(liveFrame, clone, context, snapshotInline) {
		try {
			const childDocument = frameContentDocument(liveFrame);
			if (!childDocument || !documentElement(childDocument)) return;
			const childCollector = frameContentWindow(liveFrame)?.__offprintCollector;
			const options = {
				...context.options,
				captureScope: "page"
			};
			...
			for (let index = 0; index < child.warnings.length; index += 1) arrayPush(context.warnings, child.warnings[index]);
		} catch (error) {
			if (context.budget.limitResponse(error)) throw error;
			arrayPush(context.warnings, {
	...
	function mergeInlineVisualFallbacks(frame, child, context) {
		...
		for (let index = 0; index < child.visualFallbacks.length; index += 1) {
			...
			if (target) mapSet(context.visualFallbackTargets, fallback.id, target);
		}
		return child.html;
	}
	...
	function canvasImage(live, clone) {
		const document = ownerDocument(live);
		if (!document) throw new SafeTypeError("a canvas has no owner document");
	...
	function videoImage(live, clone) {
		const document = ownerDocument(live);
		if (!document) throw new SafeTypeError("a video has no owner document");
		const image = createElement(document, "img");
		copyAttributes(clone, image);
	...
	function copyAttributes(source, destination) {
		...
		for (let index = 0; index < count; index += 1) {
			...
			if (attribute) setAttribute(destination, attributeName(attribute), attributeValue(attribute));
		}
	}
	function materializeVideo(live, clone, context) {
		...
		if (readyState >= 2 && width > 0 && height > 0) {
			...
			try {
				...
				if (!drawing) throw new SafeError("2D canvas context is unavailable");
				...
				if (reservedPayloadBytes > 0) context.budget.settlePayloadAllocation(context.reservation, reservedPayloadBytes, 0);
				if (allocatedPayloadBytes > 0) context.budget.settlePayloadAllocation(context.reservation, allocatedPayloadBytes, 0);
				if (context.budget.limitResponse(error)) throw error;
	...
	function reserveCanvasDataUrl(width, height, context) {
		...
		if (maximumBytes === null || maximumBytes > availableBytes) context.budget.rejectPayloadAllocation(context.reservation, maximumBytes ?? availableBytes + 1);
	...
	function encodeReservedCanvasDataUrl(canvas, reservedBytes, context) {
		...
		if (actualBytes === null) context.budget.rejectPayloadAllocation(context.reservation, reservedBytes + 1);
	...
	function maximumPngDataUrlBytes(width, height) {
		if (!numberIsSafeInteger(width) || width < 0 || !numberIsSafeInteger(height) || height < 0) return null;
		const rowBytes = width * 4 + 1;
		const sourceBytes = rowBytes * height;
		if (!numberIsSafeInteger(rowBytes) || !numberIsSafeInteger(sourceBytes)) return null;
		const deflateBytes = sourceBytes + mathCeil(sourceBytes / 4096) + mathCeil(sourceBytes / 16384) + mathCeil(sourceBytes / 33554432) + 13;
		const pngBytes = deflateBytes + mathCeil(deflateBytes / 65535) * 12 + 4096;
	...
	function copyCssRules(sheet, root, context, maximumBytes) {
		try {
			const usedFonts = context.options.removeUnusedFonts ? usedFontsForRoot(root, context) : void 0;
			const inheritedFontFaces = nodeType(root) === 11 && styleSheetOwner(sheet) === null ? context.documentFontFaces : void 0;
			const copied = [];
			let bytes = 0;
			const rules = rulesForStyleSheet(sheet);
			const count = cssRuleCount(rules);
			for (let index = 0; index < count; index += 1) {
				...
				if (rule && keepCssRule(rule, root, context, usedFonts, inheritedFontFaces)) {
					...
					if (bytes > maximumBytes - separatorBytes) return {
						attempted: maximumBytes + 1,
						kind: "limit"
					};
	...
	function materializeCssRules(sheet, root, context, allocatePayload) {
		...
		if (copied.kind === "limit") {
			context.budget.settlePayloadAllocation(context.reservation, unclaimedPayloadBytes, 0);
			if (allocatePayload) context.budget.rejectPayloadAllocation(context.reservation, copied.attempted);
	...
	function keepCssRule(rule, root, context, usedFonts, inheritedFontFaces) {
		...
		if (context.options.removeUnusedCss && kind === 1) {
			...
			if (!regExpTest(statefulSelector, selector)) try {
				if (!querySelector(root, selector)) return false;
			} catch {
				return true;
			}
		}
		if (kind === 5 && inheritedFontFaces && setHas(inheritedFontFaces, cssRuleText(rule))) return false;
		if (usedFonts && kind === 5 && !hasUsedFont(fontFamilies(stylePropertyValue(cssRuleDeclaration(rule), "font-family")), usedFonts)) return false;
		return true;
	}
	...
	function usedFontFamilies(root) {
		...
		for (let index = 0; index < elements.length; index += 1) {
			...
			for (let familyIndex = 0; familyIndex < elementFonts.length; familyIndex += 1) setAdd(families, elementFonts[familyIndex]);
			for (let pseudoIndex = 0; pseudoIndex < pseudoElements.length; pseudoIndex += 1) {
				...
				if (stylePropertyValue(pseudoStyle, "content") !== "none") {
					...
					for (let familyIndex = 0; familyIndex < pseudoFonts.length; familyIndex += 1) setAdd(families, pseudoFonts[familyIndex]);
				}
			}
		}
		return families;
	...
	function fontFamilies(value) {
		...
		for (let index = 0; index < matches.length; index += 1) {
			...
			if (family) arrayPush(families, family);
		}
		return families;
	}
	function hasUsedFont(families, usedFonts) {
		for (let index = 0; index < families.length; index += 1) if (setHas(usedFonts, families[index])) return true;
		return false;
	}
	...
	function applyCssom(source, clones, context) {
		...
		for (let index = 0; index < sheets.length; index += 1) {
			...
			if (!owner || nodeType(owner) !== 1) continue;
			const ownerClone = mapGet(clones, owner);
			if (!ownerClone || !isElement(ownerClone)) continue;
			...
			if (css === void 0) continue;
			if (replacesStyleText) setNodeTextContent(ownerClone, css);
			else {
	...
	function markStyleState(element, sheet) {
		...
		if (media) setAttribute(element, "media", media);
		else removeAttribute(element, "media");
		const href = styleSheetHref(sheet);
		if (href) setAttribute(element, cssBaseAttribute, href);
	}
	...
	function documentFor(root) {
		if (nodeType(root) === 9) return root;
		const document = ownerDocument(root);
		if (!document) throw new SafeTypeError("a shadow root has no owner document");
		return document;
	}
	...
	function copyShadowRoot(liveRoot, cloneHost, context) {
		...
		for (let index = 0; index < liveChildren.length; index += 1) appendChild(fragment, cloneNode(liveChildren[index], true));
		const motionRules = [];
		applyCssom(liveRoot, materializePairs(liveRoot, fragment, context, motionRules), context);
	...
	function materializePairs(liveRoot, cloneRoot, context, motionRules) {
		...
		while (stack.length > 0) {
			...
			for (let index = count - 1; index >= 0; index -= 1) arrayPush(stack, [liveChildren[index], cloneChildren[index]]);
		}
		...
		for (let pairIndex = 0; pairIndex < pairs.length; pairIndex += 1) {
			...
			if (isElement(live) && isElement(clone)) {
				...
				if (shadow) copyShadowRoot(shadow, clone, context);
				if (context.options.removeHiddenElements && isLayoutlessHiddenElement(liveElement)) removeNode(clone);
			}
		}
		return clones;
	}
	...
	function applySelectionScope(source, clones) {
		...
		if (selection) {
			...
			for (let index = 0; index < rangeCount; index += 1) {
				...
				if (!rangeCollapsed(range)) arrayPush(ranges, range);
			}
		}
		const body = documentBody(source);
		...
		const nodes = [];
		const walker = createTreeWalker(source, body, 5);
		while (treeWalkerNext(walker)) arrayPush(nodes, treeWalkerCurrent(walker));
		for (let rangeIndex = 0; rangeIndex < ranges.length; rangeIndex += 1) {
			...
			for (let nodeIndex = 0; nodeIndex < nodes.length; nodeIndex += 1) {
				...
				for (let current = node; current && current !== body; current = parentNode(current)) setAdd(selected, current);
			}
		}
		for (let index = 0; index < nodes.length; index += 1) {
			...
			if (!setHas(selected, node) && parentNode(node) && setHas(selected, parentNode(node))) {
				...
				if (clone) removeNode(clone);
			}
		}
		const cloneBody = mapGet(clones, body);
		if (cloneBody && isElement(cloneBody)) setAttribute(cloneBody, "data-offprint-selection", "");
		return {
	...
	function snapshotDocument(source, rootContext) {
		...
		for (let index = 0; index < removableSources.length; index += 1) removeNode(removableSources[index]);
		applyCssom(source, clones, context);
		const head = querySelector(clone, "head");
		if (head) appendMotionStyles(head, motionRules);
		const selection = context.options.captureScope === "selection" ? applySelectionScope(source, clones) : {
			ranges: 0,
			nodes: 0
		};
		if (rootContext.selectorTarget) applySelectorScope(source, rootContext.selectorTarget, clone, clones);
		const adoptedStyleHost = querySelector(clone, "body") ?? head;
		if (adoptedStyleHost) appendAdoptedStyles(source, adoptedStyleHost, context);
		...
		for (let originalIndex = 0; originalIndex < liveFrameOwners.length; originalIndex += 1) {
			...
			if (cloneOwner && isElement(cloneOwner) && contains(clone, cloneOwner)) {
				...
				for (let index = 0; index < mappings.length; index += 1) arrayPush(frameOwners, mappings[index]);
				retainedIndex += 1;
			}
		}
		const serialized = serializeDocumentWithRepair(source, clone, context.budget.maximumDocumentBytes(reservation));
		if (serialized.kind === "limit") context.budget.rejectPayload(serialized.attempted);
	...
	function frameOwnerMappings(originalIndex, retainedIndex, nested) {
		...
		for (let index = 0; index < nested.length; index += 1) {
			...
			for (let pathIndex = 0; pathIndex < nestedMapping.originalPath.length; pathIndex += 1) arrayPush(originalPath, nestedMapping.originalPath[pathIndex]);
			for (let pathIndex = 0; pathIndex < nestedMapping.retainedPath.length; pathIndex += 1) arrayPush(retainedPath, nestedMapping.retainedPath[pathIndex]);
			arrayPush(mappings, {
				originalPath,
				retainedPath
			});
	...
	var SnapshotLimitError = class extends Error {
		response;
	...
	const snapshotLimitErrors = new SafeWeakSet();
	function snapshotLimitResponse(error) {
		if (typeof error !== "object" || error === null || !weakSetHas(snapshotLimitErrors, error)) return;
		return error.response;
	}
	var RecursiveSnapshotBudget = class {
		captureId;
		maximumFrameDepth;
		maximumFrames;
		maximumPayloadBytes;
		...
		reserveDocument(source, frameDepth) {
			if (frameDepth > this.maximumFrameDepth) throw new SnapshotLimitError(frameDepthError(this.captureId, frameDepth, this.maximumFrameDepth));
			if (this.remainingFrames < 1) throw new SnapshotLimitError(frameLimitError(this.captureId, this.consumedFrames + 1, this.maximumFrames));
			const measured = preflightDocumentWithinLimits(source, this.remainingNodes, this.remainingPayloadBytes);
			if (measured.kind !== "ok") {
				if (measured.kind === "nodes") throw new SnapshotLimitError(nodeLimitError(this.captureId, measured.attempted, this.remainingNodes));
		...
		reservePayloadAllocation(reservation, payloadBytes) {
			if (!numberIsSafeInteger(payloadBytes) || payloadBytes < 0) throw new SafeTypeError("payloadBytes must be a non-negative safe integer");
			if (payloadBytes > this.remainingPayloadBytes) throw new SnapshotLimitError(payloadLimitError(this.captureId, this.maximumPayloadBytes, this.maximumPayloadBytes - this.remainingPayloadBytes + payloadBytes));
		...
		settlePayloadAllocation(reservation, reservedBytes, actualBytes) {
			if (!numberIsSafeInteger(reservedBytes) || reservedBytes < 0 || !numberIsSafeInteger(actualBytes) || actualBytes < 0 || reservedBytes > reservation.allocatedPayloadBytes) throw new SafeTypeError("payload allocation settlement is invalid");
		...
		commitDocument(reservation, payloadBytes) {
			...
			const adjustment = (documentPayload > 0 ? documentPayload : 0) - reservation.sourcePayloadBytes;
			const availablePayloadBytes = this.remainingPayloadBytes + reservation.allocatedPayloadBytes;
			if (adjustment > availablePayloadBytes) throw new SnapshotLimitError(payloadLimitError(this.captureId, this.maximumPayloadBytes, this.maximumPayloadBytes - availablePayloadBytes + adjustment));
	...
	function preflightDocumentWithinLimits(source, maximumNodes, maximumPayloadBytes) {
		...
		while (pending.length > 0) {
			...
			if (sibling) arrayPush(pending, sibling);
			const child = firstChild(node);
			if (child) arrayPush(pending, child);
			if (nodeType(node) !== elementNode) continue;
			if (namespaceUri(node) === htmlNamespace && localName(node) === "template") arrayPush(pending, templateContent(node));
			const shadow = observedShadowRoot(node);
			if (shadow) arrayPush(pending, shadow);
		}
		return {
			kind: "ok",
			nodes,
	...
	function measureDomNodePayload(node, maximumBytes) {
		...
		const add = (value, fixedBytes = 0) => {
			if (bytes > maximumBytes - fixedBytes) return false;
			bytes += fixedBytes;
		...
		if (type === elementNode) {
			const element = node;
			const name = localName(element) ?? "";
			if (!add(name, 2)) return null;
			const count = attributeCount(element);
			for (let index = 0; index < count; index += 1) {
				const attribute = attributeAt(element, index);
				if (attribute && (!add(attributeName(attribute), 1) || !add(attributeValue(attribute), 3))) return null;
			}
			if (!add(name, 3)) return null;
			return bytes;
		}
		if (type === 8) return add(nodeValue(node) ?? "", 7) ? bytes : null;
		if (type === 10) return add(doctypeName(node), 11) ? bytes : null;
	...
	function dispatchCollector(collector, method, arguments_) {
		switch (method) {
			case "freeze": return collector.freeze();
			case "handshake": return collector.handshake(arguments_[0], arguments_[1], arguments_[2], arguments_[3]);
			case "prepare": return collector.prepare(arguments_[0]);
			case "positionVisualFallback": return collector.positionVisualFallback(arguments_[0]);
			case "describe": return collector.describe(arguments_[0], arguments_[1]);
			case "read": return collector.read(arguments_[0], arguments_[1], arguments_[2]);
			case "acknowledge": return collector.acknowledge(arguments_[0], arguments_[1], arguments_[2]);
			case "release": return collector.release(arguments_[0], arguments_[1]);
			case "snapshotInline": return collector.snapshotInline(arguments_[0]);
			default: throw new SafeTypeError("collector method is unavailable");
		}
	}
	//#endregion
	//#region src/visual-fallback.ts
	const targets = new SafeMap();
	const fallbackTargets = objectFreeze({
		map: targets,
		clear() {
			mapClear(targets);
		},
		position(id) {
			...
			if (!target) throw new SafeTypeError("visual fallback target is unavailable");
			return positionVisualFallback(target);
		}
	});
	//#endregion
	//#region src/index.ts
	const observations = new SafeMap();
	...
	const collector = objectFreeze({
		...
		async prepare(options) {
			...
			if (!numberIsSafeInteger(options.maximumChunkBytes) || options.maximumChunkBytes < 1) throw new SafeTypeError("maximumChunkBytes must be a positive safe integer");
			if (!numberIsSafeInteger(options.maximumPayloadBytes) || options.maximumPayloadBytes < 1) return payloadLimitError(options.captureId, options.maximumPayloadBytes);
			if (!numberIsSafeInteger(options.maximumNodes) || options.maximumNodes < 0) return nodeLimitError(options.captureId, 0, options.maximumNodes);
			if (!numberIsSafeInteger(options.maximumFrames) || options.maximumFrames < 0) return frameLimitError(options.captureId, 0, options.maximumFrames);
			if (!numberIsSafeInteger(options.frameDepth) || options.frameDepth < 0 || !numberIsSafeInteger(options.maximumFrameDepth) || options.maximumFrameDepth < 0) return frameDepthError(options.captureId, options.frameDepth, options.maximumFrameDepth);
			...
			if (serialized.kind === "limit") return payloadLimitError(options.captureId, options.maximumPayloadBytes, serialized.attempted);
		...
		describe(captureId, frameId) {
			...
			if (!stored) throw new SafeTypeError("observation is not prepared");
			return {
				captureId,
				frameId,
		...
		read(captureId, frameId, sequence) {
			const stored = mapGet(observations, key(captureId, frameId));
			if (!stored || !numberIsSafeInteger(sequence) || sequence < 0 || sequence >= stored.chunkCount) throw new SafeRangeError("observation chunk is unavailable");
			...
			const payloadValues = [];
			for (let index = 0; index < typedArrayByteLength(payload); index += 1) arrayPush(payloadValues, payload[index]);
			return {
				captureId,
				frameId,
				sequence,
