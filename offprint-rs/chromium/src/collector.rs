...
use offprint_browser::{FrameObservation, FrameOwnerObservation, ObservationLimits, ObservedFrame};
use offprint_model::{
    CaptureId, ContentDigest, ContentPolicy, ErrorCode, ErrorStage, FrameId, MAXIMUM_CAPTURE_NODES,
...
use offprint_protocol::{
    ...
    CollectorMessage, ObservationDescriptor, TransferPlan, negotiate_protocol,
};
...
pub(crate) async fn collect_frame_observation_with_metadata(
    ...
    let prepare = CollectorCommand::Prepare {
        ...
        preserve_password_values: content.preserve_password_values,
        capture_scope: content.scope,
        selector: (limits.frame_depth == 0)
            .then(|| content.selector.clone())
            .flatten(),
        remove_unused_css: content.optimizations.remove_unused_css,
        remove_unused_fonts: content.optimizations.remove_unused_fonts,
        remove_hidden_elements: content.optimizations.remove_hidden_elements,
    };
    ...
    let collection = async {
        ...
        let observation =
            serde_json::from_slice::<FrameObservation>(&payload).map_err(|error| {
    ...
    match (collection, release) {
        (Ok((observation, encoded_bytes)), Ok(_)) => Ok(ObservedFrame {
            observation,
            encoded_bytes,
        }),
...
fn collector_shape_error(message: impl Into<String>) -> OffprintError {
    OffprintError::new(
...
#[cfg(test)]
mod tests {
    use offprint_browser::FrameOwnerObservation;
    use offprint_model::{CaptureId, MAXIMUM_CAPTURE_NODES};
    use serde_json::json;
    ...
    fn frame_owner_paths_are_nonempty_unique_and_depth_aligned() {
        ...
        assert_eq!(
            ...
            Err("offprint.collector.protocol_shape")
        );
        assert_eq!(
