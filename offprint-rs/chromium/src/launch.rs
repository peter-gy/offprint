...
use offprint_model::{BrowserEnvironment, ErrorStage, OffprintError, PortablePath, Result};
use processkit::{
...
impl ChromiumProcess {
    pub async fn launch(options: ChromiumLaunchOptions) -> Result<Self> {
        ...
        let profile = tempfile::Builder::new()
            .prefix("offprint-browser-")
            .tempdir()
            .map_err(|error| {
                browser_error(
                    "offprint.browser.profile",
        ...
        .map_err(|error| process_error("offprint.browser.containment", "create", error))?;
        let process = process_group.start(&command).await.map_err(|error| {
            process_error("offprint.browser.launch", "launch Chromium", error).retryable(true)
        })?;
...
fn map_process_task(
    ...
    outcome.map_err(|error| {
        with_browser_diagnostics(
            process_error("offprint.browser.shutdown", "reap Chromium process", error)
                .retryable(true),
            diagnostics,
        )
    })?;
...
async fn wait_for_endpoint(group: &ProcessGroup, profile: &std::path::Path) -> Result<Url> {
    ...
    loop {
        ...
        if started.elapsed() >= LAUNCH_TIMEOUT {
            return Err(browser_error(
                "offprint.browser.launch_timeout",
...
fn parse_active_port(contents: &str) -> Result<Option<Url>> {
    ...
    let port = port.parse::<u16>().map_err(|error| {
        browser_error(
            "offprint.browser.devtools_port",
...
mod tests {
    ...
    fn chromium_stderr_diagnostics_are_redacted() {
        let profile = "/tmp/offprint-browser-secret";
        let line = format!(
