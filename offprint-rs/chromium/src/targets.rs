...
const CONTAINMENT_SCRIPT: &str = r#"(() => {
    const block = (name) => {
        ...
        const Blocked = function () {
            throw new DOMException("Offprint blocked a direct network transport.", "SecurityError");
        };
...
impl FrameTargetManager {
    ...
    pub(crate) async fn error(&self) -> Option<OffprintError> {
...
fn frame_limit_error(maximum: u32) -> OffprintError {
    OffprintError::new(
        "offprint.frame.limit",
        ErrorStage::Collection,
...
async fn set_target_error(state: &Mutex<TargetState>, error: OffprintError) {
    let mut state = state.lock().await;
    if state.error.is_none() {
