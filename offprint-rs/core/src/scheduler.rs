...
use offprint_model::{
    BatchRequest, BatchResult, BrowserSpec, CaptureArtifact, CaptureOutput, CaptureRequest,
    ConflictPolicy, ContentDigest, CrawlFrontierItem, CrawlPageOutcome, CrawlRequest, CrawlResult,
    ErrorStage, OffprintError, PortablePath, Result, ResumeJobStatus, ResumeManifest, ScheduleKind,
        ...
        let mut outcomes = manifest
            ...
            .map(|(id, record)| {
                let url = record.url.clone().ok_or_else(|| {
                    scheduler_error(
                        "offprint.scheduler.manifest_shape",
                        ErrorStage::Internal,
...
fn validate_batch_jobs(request: &BatchRequest) -> Result<()> {
    if request.schema_version != offprint_model::PUBLIC_SCHEMA_VERSION {
        return Err(scheduler_error(
            ...
            ErrorStage::Validation,
            "batch request schema version is incompatible",
        ));
    }
    if request.jobs.is_empty() || request.jobs.len() > MAXIMUM_BATCH_JOBS {
        return Err(scheduler_error(
            "offprint.scheduler.job_count",
            ErrorStage::Validation,
    ...
    for job in &request.jobs {
        ...
        if request.resume.is_some() && !job.request.credentials.is_empty() {
            return Err(scheduler_error(
                "offprint.scheduler.resume_credentials",
                ErrorStage::Validation,
                "resumable batch jobs require empty capture credentials",
            ));
        }
        if request.resume.is_some() && !matches!(job.request.output, CaptureOutput::File { .. }) {
            return Err(scheduler_error(
                "offprint.scheduler.resume_target",
                ErrorStage::Validation,
...
fn validate_crawl_request(request: &CrawlRequest) -> Result<()> {
    if request.schema_version != offprint_model::PUBLIC_SCHEMA_VERSION {
        return Err(scheduler_error(
            "offprint.input.schema_version",
            ErrorStage::Validation,
            "crawl request schema version is incompatible",
        ));
    }
    validate_concurrency(request.concurrency)?;
...
fn validate_concurrency(concurrency: u16) -> Result<()> {
    if concurrency == 0 || concurrency > MAXIMUM_CONCURRENCY {
        return Err(scheduler_error(
            "offprint.scheduler.concurrency",
            ErrorStage::Validation,
...
fn crawl_capture_request(request: &CrawlRequest, item: &CrawlFrontierItem) -> CaptureRequest {
    ...
    capture.output = CaptureOutput::File {
        path: crawl_output_path(request, item),
...
fn crawl_output_path(request: &CrawlRequest, item: &CrawlFrontierItem) -> PortablePath {
    ...
    let stem = offprint_artifact::portable_file_stem(hint);
...
async fn crawl_links(
    result: &offprint_model::CaptureReceipt,
    requested_url: &Url,
    same_origin: bool,
    seed_url: &Url,
) -> Result<Vec<Url>> {
    let bytes = match &result.artifact {
        CaptureArtifact::File { path, .. } => tokio::fs::read(path).await.map_err(|error| {
            scheduler_error(
                "offprint.scheduler.artifact_read",
                ErrorStage::Internal,
                format!("failed to read a captured crawl artifact: {error}"),
            )
        })?,
        CaptureArtifact::Bytes { content, .. } => content.clone(),
    };
    let manifest = offprint_html::inspect_html(&bytes)?;
    let base =
...
mod tests {
    use offprint_model::{
        ...
        PortablePath, RequestHeader, ResumeOptions, SecretString, VerificationMode,
    };
    use url::Url;
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(
            ...
            Err("offprint.input.browser_selection")
        );
        Ok(())
    }
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(
            validate_batch_jobs(&request)
                .as_ref()
                .map_err(|error| error.code.as_str()),
            Err("offprint.scheduler.resume_credentials")
        );
        Ok(())
    }
