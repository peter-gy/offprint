...
use offprint_browser::{
    AttachedFrame, FrameObservation, NetworkGuard, ObservationLimits, PageSession,
};
...
use offprint_model::{
    ...
    ErrorStage, Milliseconds, OffprintError, RedactedUrl, RedactionPolicy, ResourceRecord,
...
use super::resource_materialization::{
    materialize_visual_fallbacks, validate_capture_url, validate_file_resource,
};
...
pub(super) async fn capture(
    ...
    cancellable(
        job.cancellation(),
        validate_capture_url(guard, &final_url, &request.content.allowed_file_roots),
    )
    .await?;
    let navigation = navigation_started.elapsed();

    transition(job, CaptureStatus::WaitingForReadiness).await?;
    ...
    for frame in &attached_frames {
        if !frame.url.starts_with("file:") {
            continue;
        }
        if matches!(request.browser, offprint_model::BrowserSpec::Remote(_)) {
            return Err(OffprintError::new(
                "offprint.input.remote_file",
                ErrorStage::Validation,
                "remote browser frames cannot address the coordinator filesystem",
            ));
        }
        let url = Url::parse(&frame.url).map_err(|error| {
            OffprintError::new(
                "offprint.input.file_root",
                ErrorStage::Collection,
                format!("attached frame returned an invalid file URL: {error}"),
            )
        })?;
        cancellable(
            job.cancellation(),
            validate_file_resource(&url, &request.content.allowed_file_roots),
        )
        .await?;
    }
    ...
    if request.content.scope == offprint_model::CaptureScope::Selection {
        let top_frame = frame_descriptors.first().ok_or_else(|| {
            OffprintError::new(
                "offprint.frame.collection",
                ErrorStage::Collection,
    ...
    for frame in &collection_order {
        ...
        for warning in &observation.warnings {
            if warning.code == "offprint.frame.cross_origin" && captured_cross_origin_frames > 0 {
        ...
        let source = {
            ...
            offprint_document::serialize_document(&document).map_err(|error| {
                OffprintError::new(
        ...
        let html = render_frame_document(
            ...
            FrameRenderContext {
                ...
                browser_frame_id: &frame.browser_frame_id,
                guard,
                request,
                job,
                capture_id,
    ...
    let timings = CaptureTimings {
        ...
        readiness: Milliseconds::from(settle),
...
async fn describe_frames(
    ...
    let mut descriptors = vec![FrameDescriptor {
        frame_id: offprint_model::FrameId::new(1),
        session_id: main_session.clone(),
        browser_frame_id: top_browser_frame_id.to_owned(),
        parent_session_id: None,
        owner_path: None,
        depth: 0,
    }];
    let mut known = BTreeMap::from([(main_session, (offprint_model::FrameId::new(1), 0_u16))]);
    ...
    while !pending.is_empty() {
        ...
        for candidate in ready {
            ...
            let frame_id = offprint_model::FrameId::new(next_id);
            ...
            descriptors.push(FrameDescriptor {
                ...
                browser_frame_id: candidate.frame.target_id,
...
fn frame_limit_error(maximum_frames: u32) -> OffprintError {
    OffprintError::new(
        "offprint.frame.limit",
        ErrorStage::Collection,
...
fn retain_observed_frame_descriptors(
    ...
    observations: &BTreeMap<String, FrameObservation>,
) -> Result<Vec<FrameDescriptor>> {
...
fn observation_document(observation: &FrameObservation) -> Result<Document> {
    ...
    offprint_document::set_document_scroll_state(
        &mut document,
...
mod tests {
    ...
    use offprint_browser::{
        FrameObservation, FrameOwnerObservation, ObservationViewport, SelectionObservation,
    };
    ...
    fn retained_frame_descriptors_remap_nested_pruned_owner_paths() {
        let descriptor = |id, session: &str, parent: Option<&str>, owner, depth| FrameDescriptor {
            frame_id: offprint_model::FrameId::new(id),
            session_id: session.to_owned(),
            browser_frame_id: format!("frame-{id}"),
        ...
        let observation = |frame_owners| FrameObservation {
            doctype: String::new(),
    ...
    fn observation_scroll_state_is_materialized_on_the_document_root() {
        ...
        let html = observation_document(&observation)
            .and_then(|document| {
                offprint_document::serialize_document(&document).map_err(|error| {
                    offprint_model::OffprintError::new(
                        "offprint.artifact.serialize",
                        offprint_model::ErrorStage::Transform,
                        error.to_string(),
                    )
                })
            })
        ...
        assert!(html.is_ok_and(|html| {
            html.contains(r#"data-offprint-scroll-x="24.5""#)
                && html.contains(r#"data-offprint-scroll-y="480""#)
        }));
    }
}
