...
use offprint::{
    BrowserCookie, BrowserEnvironment, CaptureArtifact, CaptureOutput, CaptureReceipt,
    CaptureRequest, CaptureScope, CaptureStatus, ConflictPolicy, ContentDigest, ContentPolicy,
    Milliseconds, MissingResourcePolicy, NetworkPolicy, Offprint, OptimizationPolicy, PortablePath,
    ReadinessMode, RequestHeader, SecretString,
};
...
async fn static_article_with_font_round_trips() -> TestResult {
    ...
    let manifest = offprint_html::inspect_html(&captured.content)?;
    assert!(manifest.resource_records.iter().any(|record| {
        matches!(
            &record.outcome,
            offprint::ResourceOutcome::Embedded { media_type, .. }
    ...
    for suffix in ["/styles/fixture.woff2", "/styles/background.svg"] {
        assert!(manifest.resource_records.iter().any(|record| {
            record.requested_url.as_str().ends_with(suffix)
                && matches!(record.outcome, offprint::ResourceOutcome::Embedded { .. })
        }));
    }
...
async fn adopted_stylesheet_reuses_the_document_font_face() -> TestResult {
    ...
    assert!(html.contains("data-offprint-adopted"), "{html}");
...
async fn browser_state_fixture_round_trips() -> TestResult {
    ...
    assert!(
        html.contains("data-offprint-shadow-mode=\"closed\""),
        "{html}"
    );
    ...
    assert!(html.matches("data-offprint-canvas").count() >= 2, "{html}");
    assert!(html.contains("data-offprint-media-poster"), "{html}");
...
async fn rendered_state_reopens_across_light_and_shadow_trees() -> TestResult {
    ...
    let html = String::from_utf8_lossy(&captured.content);
    assert!(html.contains("data-offprint-scroll-left=\"30\""), "{html}");
    assert!(html.contains("data-offprint-scroll-top=\"70\""), "{html}");
    assert!(
        html.contains("data-offprint-shadow-mode=\"closed\""),
        "{html}"
    );
...
async fn permanent_connections_do_not_block_render_idle() -> TestResult {
    ...
    let captured = tokio::time::timeout(
        ...
        capture_bytes(&offprint, server.url("/")?, NetworkPolicy::Standard),
    )
    .await??;
...
async fn frame_fixture_round_trips() -> TestResult {
    ...
    let captured = capture_bytes(&offprint, parent.url("/")?, NetworkPolicy::Unrestricted).await?;
    ...
    assert_eq!(html.matches("data-offprint-frame-id=").count(), 4);
    assert!(captured.result.warnings.iter().all(|warning| {
        !matches!(
            warning.code.as_str(),
            "offprint.frame.cross_origin" | "offprint.frame.collection"
        )
    }));
...
async fn request_policy_fixture_round_trips() -> TestResult {
        ...
        .register_handler("/protected.svg", |request| {
            let authenticated = request
                ...
                && request
                    .headers
                    .get("cookie")
                    .is_some_and(|value| value.contains("offprint_session=cookie-secret"));
            if authenticated {
    ...
    request.output = CaptureOutput::memory(8 * 1024 * 1024);
    ...
    request.credentials.cookies.push(BrowserCookie {
        name: "offprint_session".to_owned(),
    ...
    assert!(manifest.resource_records.iter().all(|record| {
        matches!(record.outcome, offprint::ResourceOutcome::Embedded { .. })
            && record.provenance.as_ref().is_some_and(|provenance| {
                provenance.source == offprint::ResourceRetrievalSource::ObservedResponse
            })
    }));
...
async fn partial_resource_has_a_typed_outcome() -> TestResult {
    ...
    assert!(matches!(
        ...
        offprint::ResourceOutcome::Failed { .. }
    ));
...
async fn oversized_resource_fails_before_artifact_commit() -> TestResult {
    ...
    for (root, resource_url, expected_code) in [
        ("/signed", &signed_url, "offprint.resource.limit"),
        ("/userinfo", &userinfo_url, "offprint.resource.load"),
    ] {
        let mut request = CaptureRequest::builder(server.url(root)?.as_str())?.build()?;
        request.output = CaptureOutput::memory(1024 * 1024);
        ...
        let error = match offprint.captures().start(request).await?.result().await {
            Ok(result) => {
...
async fn explicit_output_conflict_preserves_the_destination() -> TestResult {
    ...
    request.output = CaptureOutput::file(PortablePath::from_path_buf(destination.clone())?);
    request.output = request.output.with_conflict(ConflictPolicy::Fail);
    let offprint = Offprint::builder().build()?;
...
async fn atomic_replacement_commits_the_verified_artifact() -> TestResult {
    ...
    offprint_html::verify_static(&content)?;
    offprint.close().await?;
    server.close().await;
    Ok(())
}
...
async fn cancellation_rolls_back_every_pipeline_stage() -> TestResult {
    ...
    for stage in [
        ...
        CaptureStatus::WaitingForReadiness,
        ...
        .await?;
        let Err(error) = job.result().await else {
            return Err(std::io::Error::other(format!(
...
async fn visual_optimizers_reduce_resources_and_preserve_rendering() -> TestResult {
    ...
    let offprint = Offprint::builder().build()?;
    let baseline = capture_bytes_with_policy(&offprint, url.clone(), ContentPolicy::default())
        .await
        .map_err(|error| std::io::Error::other(format!("baseline capture failed: {error:?}")))?;
    let optimized_policy = ContentPolicy {
    ...
    let optimized = capture_bytes_with_policy(&offprint, url, optimized_policy)
        .await
...
fn artifact_bytes(result: &CaptureReceipt) -> TestResult<Vec<u8>> {
    match &result.artifact {
        CaptureArtifact::Bytes { content, .. } => Ok(content.clone()),
...
fn assert_success_result(result: &CaptureReceipt) {
