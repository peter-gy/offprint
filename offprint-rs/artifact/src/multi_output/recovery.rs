...
fn rollback_entries(
    ...
    for (index, entry) in entries.iter().enumerate().rev() {
        ...
        if backup_exists {
            if destination_exists {
                if path_matches_entry(&destination, entry, limits) {
                    ...
                    failures.push(format!(
                        "destination `{}` changed after Offprint created its backup",
                        destination.display()
                    ));
                    continue;
                }
        ...
        if path_matches_entry(&destination, entry, limits) {
            ...
            failures.push(format!(
                "new destination `{}` does not match the Offprint journal",
                destination.display()
            ));
        }
    }
    ...
    if previous_exists {
        if output_exists {
            ...
            if complete || empty {
                ...
                return vec![
                    "output directory changed after Offprint retained the previous directory"
                        .to_owned(),
                ];
            }
        }
...
fn recovery_io_error(path: &Path, message: &'static str, error: std::io::Error) -> OffprintError {
    OffprintError::new(
        "offprint.export.output",
        ErrorStage::Commit,
