...
use offprint_model::{ConflictPolicy, ContentDigest, ErrorStage, OffprintError, Result};
...
pub(super) fn plan_destinations(
    ...
    for artifact in prepared {
        ...
        plans.push(DestinationPlan {
            ...
            entry: JournalEntry {
                destination_name,
                format: artifact.verification().format,
                output_kind,
                existed,
                previous_kind,
    ...
    match conflict {
        ...
        ConflictPolicy::Uniquify => {
            ...
            Err(OffprintError::new(
                ...
                ErrorStage::Validation,
                "portable artifact format filename suffixes are exhausted",
            ))
        }
    }
}
...
fn validate_file(
    ...
    if metadata.len() > maximum_bytes || metadata.len() != expected_bytes {
        return Err(plan_error(
            ErrorStage::Commit,
            "staged artifact format byte count changed before commit",
        ));
    }
    ...
    if sha256 != expected_sha256 {
        return Err(OffprintError::new(
            ...
            ErrorStage::Commit,
            "staged artifact format digest changed before commit",
        ));
    }
    Ok(())
}
...
fn hash_file(path: &Path, maximum_bytes: u64) -> Result<(u64, ContentDigest)> {
    let mut file = File::open(path).map_err(|error| {
        plan_io_error(
            ErrorStage::Commit,
            "failed to read a staged artifact format",
            error,
        )
    })?;
    let mut hasher = Sha256::new();
    ...
    loop {
        ...
        if total > maximum_bytes {
            return Err(OffprintError::new(
                "offprint.export.size",
                ErrorStage::Commit,
                "staged artifact format exceeds the export byte limit",
            ));
        }
...
fn direct_regular_file(path: &Path, stage: ErrorStage) -> Result<fs::Metadata> {
    let metadata = fs::symlink_metadata(path).map_err(|error| {
        plan_io_error(stage, "failed to inspect an artifact format file", error)
    })?;
    if metadata.file_type().is_symlink() || !metadata.is_file() {
        return Err(plan_error(
            stage,
            "artifact format must be a directly addressed regular file",
        ));
    }
    Ok(metadata)
}
...
fn output_exists_error(path: &Path) -> OffprintError {
    OffprintError::new(
...
fn plan_error(stage: ErrorStage, message: impl Into<String>) -> OffprintError {
    OffprintError::new("offprint.export.output", stage, message)
}

fn plan_io_error(stage: ErrorStage, message: &'static str, error: std::io::Error) -> OffprintError {
