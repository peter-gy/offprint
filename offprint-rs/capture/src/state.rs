use offprint_model::{CaptureStatus, ErrorStage, OffprintError, Result};
    ...
    pub fn transition(&mut self, next: CaptureStatus) -> Result<()> {
        if !can_transition(self.status, next) {
            return Err(OffprintError::new(
                "offprint.runtime.state_transition",
                ErrorStage::Internal,
                format!(
...
fn can_transition(current: CaptureStatus, next: CaptureStatus) -> bool {
    use CaptureStatus::{
        ...
        ResolvingResources, Succeeded, Transforming, Validating, Verifying, WaitingForBrowser,
        WaitingForReadiness,
    };
    matches!(
        (current, next),
        (Created, Validating)
            ...
            | (Navigating, WaitingForReadiness)
            | (WaitingForReadiness, Collecting)
...
#[cfg(test)]
mod tests {
    use offprint_model::CaptureStatus;
    ...
    fn status_strategy() -> impl Strategy<Value = CaptureStatus> {
        prop_oneof![
            ...
            Just(CaptureStatus::WaitingForReadiness),
