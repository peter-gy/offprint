...
use offprint::{
    CaptureJob, CaptureReceipt, ErrorStage, Offprint, OffprintError, Result, VerificationReport,
};
...
where
    F: Future<Output = Result<VerificationReport>>,
    ...
    F: Future<Output = std::io::Result<()>>,
{
    ...
    let mut result = Box::pin(job.result());
        ...
        tokio::select! {
            ...
            result = &mut result => {
                if interrupted {
                    ...
                    let mut error = OffprintError::new(
            ...
            event = events.next(), if events_open => {
                match event {
                    Some(event) if !options.quiet && !options.json => {
...
fn interrupted_error(operation: &str) -> OffprintError {
    OffprintError::new(
        "offprint.runtime.interrupted",
        ErrorStage::Shutdown,
...
fn signal_error(error: std::io::Error) -> OffprintError {
    OffprintError::new(
        "offprint.runtime.signal",
        ErrorStage::Shutdown,
        format!("failed to listen for interruption: {error}"),
    )
}

pub(super) async fn finish_runtime<T>(offprint: &Offprint, operation: Result<T>) -> Result<T> {
    let close = offprint.close().await;
