    ...
    for record in records:
        ...
        processes.append(
            ProcessRecord(
                ...
                command_line=(
                    command_line if isinstance(command_line, str) else ""
                ),
            )
        )
    return processes
...
def _owned_processes(
    ...
    while True:
        descendants = {
            process.pid
            for process in processes
            if process.parent_pid in owned
        }
...
def owned_processes(
    ...
    processes = (
        _windows_processes()
        if sys.platform == "win32"
        else _unix_processes()
    )
    tracked = set(tracked_pids)
    owned = {
        process.pid: process
        for process in _owned_processes(directory, processes)
    }
    owned.update(
        (process.pid, process)
        for process in processes
...
def wait_for_lifecycle_cleanup(
    ...
    while True:
        ...
        time.sleep(0.05)
    process_summary = [
        f"{process.pid}:{process.name or process.command_line}"
        for process in processes
    ]
    raise RuntimeError(
    ...
    try:
        while not ready.exists():
            if process.poll() is not None:
                ...
                errors.append(
                    f"{scenario} exited before reporting a live browser"
                )
                break
            if time.monotonic() >= deadline:
                errors.append(
                    f"{scenario} timed out before reporting a live browser"
                )
                break
            time.sleep(0.025)
        ...
        if not errors:
            ...
            if not profiles:
                errors.append(
                    f"{scenario} did not expose its owned browser profile"
                )
