...
class OffprintError extends Error {
  constructor(record) {
    ...
    this.source = record.source ? OffprintError.fromRecord(record.source) : undefined;
  }
...
function translateError(error) {
  ...
  if (marker >= 0) {
    try {
      return OffprintError.fromRecord(JSON.parse(message.slice(marker + ERROR_MARKER.length)));
    } catch {
      return error;
    }
  }
...
const finalizer = new FinalizationRegistry(({ nativeOffprint, serviceToken }) => {
...
class CaptureEvents {
  #native;
  // Retain the root service until this child can release its native work.
  ...
  constructor(options) {
    try {
      ...
      if (this.#native.initializationError) {
        throw OffprintError.fromRecord(this.#native.initializationError);
      }
    } catch (error) {
      throw translateError(error);
    }
