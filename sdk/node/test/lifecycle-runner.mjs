...
export async function runLifecycleScenario({
  ...
  const output = [];
  const child = spawn(process.execPath, ["--expose-gc", childScript, scenario], {
    env: {
      ...process.env,
      ...environment,
  ...
  try {
    while (!(await pathExists(ready))) {
      if (exited) {
        throw new Error(`${scenario} exited before reporting a live browser`);
      }
      if (Date.now() >= deadline) {
        throw new Error(`${scenario} timed out before reporting a live browser`);
      }
    ...
    if (processes.length === 0) {
      throw new Error(`${scenario} did not expose its owned browser process tree`);
    }
    ...
    if (status.code !== 0) {
      throw new Error(`${scenario} ended with ${JSON.stringify(status)}`);
    }
  } catch (error) {
    errors.push(error);
  } finally {
