...
async function fixture(name) {
  return JSON.parse(await readFile(new URL(name, examples), "utf8"));
}
...
describe("canonical contract fixtures", () => {
  test("the generated inventory resolves every record", async () => {
    ...
    for (const name of index.files) {
      expect(await fixture(name)).toEqual(expect.any(Object));
    }
  });
  ...
  test("public records use camel-case field names", async () => {
    ...
    for (const name of index.files) {
      expect(objectKeys(await fixture(name)).filter((key) => key.includes("_"))).toEqual([]);
    }
  });
  ...
  test("event fields match the Node.js job iterator contract", async () => {
    ...
    expect(event.captureId).toBeTypeOf("string");
    expect(event.completed).toBeTypeOf("number");
  });
  ...
  test("binding contracts preserve fields, enums, optionality, and defaults", async () => {
    ...
    const eventContract = inventory.contracts.find((contract) => contract.name === "CaptureEvent");
    ...
    const eventType = eventContract.enums.find(
      (record) => record.path.endsWith("/properties/type") && record.values.includes(event.type),
    );
    const variantPath = eventType.path.slice(0, -"/properties/type".length);
    expect(
    ...
    expect(
      resultContract.fields
        .filter(
          (field) => field.path.startsWith("#/properties/") && field.path.split("/").length === 3,
        )
        .map((field) => field.name)
        .sort(),
    ).toEqual(Object.keys(result).sort());
    expect(resultContract.fields.find((field) => field.name === "warnings")).toMatchObject({
      required: false,
      nullable: false,
      default: [],
    });
