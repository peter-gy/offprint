...
assert(["retained-job", "explicit-close", "abandoned-close", "host-exit"].includes(scenario));

const server = createServer((_request, response) => {
  const body = "<!doctype html><title>Lifecycle</title><h1>active capture</h1>";
...
const fixturePath =
  process.env.OFFPRINT_CAPTURE_REQUEST ??
...
request.output.path = join(process.env.TMPDIR, `${scenario}.html`);
...
const continuePath = process.env.OFFPRINT_LIFECYCLE_CONTINUE;
