...
beforeAll(async () => {
  ...
  server = createServer((request, response) => {
    if (request.url === "/asset.svg") {
      ...
      response.end(`<svg xmlns="http://www.w3.org/2000/svg" width="8" height="8">
        <rect width="8" height="8" fill="rgb(24, 96, 160)"/>
      </svg>`);
      return;
    }
    response.writeHead(200, { "content-type": "text/html; charset=utf-8" });
    response.end(`<!doctype html><html><head><title>Node binding</title></head>
  ...
  try {
    return await Promise.race([
      ...
      new Promise((_, reject) => {
        timeout = setTimeout(
          () => reject(new Error("timed out waiting for a capture event")),
          15_000,
        );
      }),
    ]);
...
test("captures, inspects, emits events, cancels, and closes", async () => {
  ...
  try {
    ...
    for (const artifact of exported.artifacts) {
      const verified = await offprint.artifacts.verifyFormat(artifact.path, artifact.format);
