...
def _install_test_panic_hook(
    ...
    owner._test_panic = invoke_test_panic
...
def _json_options(
    ...
    for key, value in options.items():
        ...
        converted[native_name] = (
            os.fspath(value) if key in path_options and isinstance(value, os.PathLike) else value
        )
...
def _finalize_native(token: int, native: NativeOffprint) -> None:
    ...
    task.add_done_callback(lambda completed: _consume_finalizer_result(token, completed))
...
class CaptureService:
    ...
    async def start(self, request: Mapping[str, Any]) -> CaptureJob:
        native_job = await _invoke(self._native.start(json.dumps(request, separators=(",", ":"))))
        return CaptureJob(native_job, self._owner)

    async def batch(self, request: Mapping[str, Any]) -> dict[str, Any]:
        value = await _invoke(self._native.batch_json(json.dumps(request, separators=(",", ":"))))
        return _decode_record(value)

    async def crawl(self, request: Mapping[str, Any]) -> dict[str, Any]:
        value = await _invoke(self._native.crawl_json(json.dumps(request, separators=(",", ":"))))
        return _decode_record(value)
...
class ArtifactService:
    ...
    async def inspect(self, path: str | os.PathLike[str]) -> dict[str, Any]:
        return _decode_record(await _invoke(self._native.inspect_json(os.fspath(path))))
    ...
    async def verify(
        ...
        *,
        ...
        return _decode_record(await _invoke(self._native.verify_json(os.fspath(path), options)))
    ...
    async def export(
        ...
        return _decode_record(
            await _invoke(self._native.export_json(os.fspath(path), request_json))
        )
    ...
    async def verify_format(
        ...
        return _decode_record(
            await _invoke(self._native.verify_format_json(os.fspath(path), format))
        )
...
class BrowserService:
    ...
    async def ensure(self) -> dict[str, Any]:
        return _decode_record(await _invoke(self._native.ensure_browser_json()))
    ...
    async def install(self, revision: str | None = None) -> dict[str, Any]:
        return _decode_record(await _invoke(self._native.install_browser_json(revision)))
    ...
    async def remove(
        ...
        *,
        ...
        return _decode_record(await _invoke(self._native.remove_browser_json(revision, force)))
...
class Offprint:
    _test_panic: Callable[[], Awaitable[None]]
