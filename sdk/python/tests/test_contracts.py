...
def object_keys(value: object) -> list[str]:
    ...
    if isinstance(value, dict):
        return [
            ...
            *[key for nested in value.values() for key in object_keys(nested)],
        ]
    return []
...
def test_public_records_use_camel_case_field_names() -> None:
    ...
    for name in index["files"]:
        assert [key for key in object_keys(fixture(str(name))) if "_" in key] == []
...
def test_binding_contracts_preserve_fields_enums_and_defaults() -> None:
    ...
    contracts = {record["name"]: record for record in inventory["contracts"]}
    ...
    event_type = next(
        ...
        if record["path"].endswith("/properties/type") and event["type"] in record["values"]
    )
    ...
    assert sorted(
        ...
        if field["path"].startswith("#/properties/") and len(field["path"].split("/")) == 3
    ) == sorted(result)
    warnings = next(
        field for field in result_contract["fields"] if field["path"] == "#/properties/warnings"
    )
    assert warnings == {
...
def test_cli_json_output_records_resolve_to_canonical_schemas() -> None:
    ...
    for command in docs["commands"]:
        ...
        assert all(success_schema in files for success_schema in command["successSchemas"])
