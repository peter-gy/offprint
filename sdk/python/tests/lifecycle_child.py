...
from offprint import Offprint, OffprintOptions


class Handler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        body = b"<!doctype html><title>Lifecycle</title><h1>active capture</h1>"
    ...
    try:
        fixture_path = Path(
            os.environ.get(
                ...
                Path(__file__).parents[3] / "schemas" / "examples" / "capture-request.json",
            )
        )
        request = json.loads(fixture_path.read_text(encoding="utf-8"))
        request["url"] = f"http://127.0.0.1:{server.server_port}/"
        request["output"]["path"] = str(Path(os.environ["TMPDIR"]) / f"{scenario}.html")
        request["readiness"]["delay"] = 30_000
        browser_path = os.environ.get("OFFPRINT_PACKAGE_BROWSER_PATH")
        options: OffprintOptions | None = {"browser_path": browser_path} if browser_path else None
        offprint = Offprint(options)
